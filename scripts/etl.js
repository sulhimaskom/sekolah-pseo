...
const { parseCsv, writeCsv, sanitize, validateLatLon, terminate } = require('./utils');
...
async function run() {
  ...
  try {
    ...
    terminate(
      `Raw data file not found at ${rawPath}. Please ensure the data file exists. Error: ${error.message}`
    );
  }
  ...
  try {
    ...
    if (error.name === 'IntegrationError') {
      terminate(`Integration error: ${error.code} - ${error.message}`);
    } else {
