# Issue Records — 204th Batch Delta (244th run, 2026-08-18)

**Ledger**: 204th batch delta. **Trigger**: Phase 0 → 0 PRs / 0 issues → Phase 1 audit (244th).

**F002 status**: GraphQL "Resource not accessible by integration (createIssue)" — HTTP-equivalent **403 CONCLUSIVE (47th)** — token lacks `issues: write`. GitHub-native issue creation remains blocked (held since 222nd record). All findings recorded here in the ledger, the established output channel.

## Finding status table (244th basis)

| Finding | Category | Priority | Status | Observation |
| ------- | -------- | -------- | ------ | ----------- |
| F002    | ci       | P1       | HELD   | Token lacks `issues: write` — **403 CONCLUSIVE (47th)** |
| F004    | security | P2       | HELD   | **57 refs / 10 names** (workflows `*.yml` only) — byte-identical to 243rd basis, **zero growth** |
| F005    | docs     | P3       | HELD   | **128 files** (92nd obs): 127 ledger + SECURITY_AUDIT_NOTE.md — **ledger-only, zero source/active-doc drift** (F230 fix held; +3 = 243rd run records) |
...
| F037    | security | P1       | HELD   | 12 workflow violations (2 CRITICAL + 10 HIGH), **145th obs**, push-blocked F050 (22nd documented pass) |
| F038    | ci       | P1       | HELD   | orchestrator workflow **8/8 failed — ~60 days** |
| F044    | security | P2       | HELD   | secret over-scoping (F004 cluster) — held at 57 refs |
| F050    | ci       | P1       | HELD   | GitHub App token lacks `workflows` permission — blocks workflow-file delivery (22nd push-block pass) |
| F063    | ci       | P1       | HELD (IMPROVING) | on-pull: **14 consecutive completed successes + this-run in-progress; no NEW failure** (17:20Z→07:31Z; streak extended 9→14) |
...
| F228    | docs     | P3       | **RESOLVED maintained** | README.md:296-302, SECURITY.md:48, deployment.md:64 accurate at HEAD — **6th clean obs, PR #798 fix held** |
| F229    | ci       | P1       | HELD   | TASK-102 fix commit `0056ad8` UNREACHABLE (9th obs); on-pull.yml:63 unhardened install (no pipefail/retry) |
| F230    | docs     | P3       | **RESOLVED maintained** | blueprint.md/task.md prettier-clean at HEAD (8th clean obs) |

## New findings detail

**None this window.** F228 maintained RESOLVED (6th clean obs). F230 maintained RESOLVED (8th obs). F063 window IMPROVING — completed-success streak extended from 9 to **14 consecutive successes** (no new failure since the 16:18Z curl-429 flake; 08:26Z run = this run, in progress at audit time). F004 zero growth (byte-identical workflows). F026/F017/F028/F019 maintained RESOLVED. F064 re-surfaced: lint-staged@17.3.0 emits EBADENGINE (requires node ≥22.22.1) against the node-20 runtime at `npm ci`.
...
1. **F229/F063/F037/F038 (P1, BLOCKED)**: workflow hardening (pipefail/retry install, trigger gating, secret scoping, `--admin` removal) requires `workflows: write` — token lacks it (F050, 22nd documented pass). Record and hold; deliverable only with a workflows-enabled token.
...
4. **F005 (P3)**: ledger files exempt by convention (92nd obs) — no action.
