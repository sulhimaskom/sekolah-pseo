# Issue Records — 239th Batch Delta (279th run, 2026-08-20)

**Ledger**: 239th batch delta. **Trigger**: Phase 0 → 1 open PR (#840, 278th ledger) → PR HANDLER MODE: **merged #840** → Phase 0 re-probe 0 open PRs / 0 open issues → Phase 0.3 EMPTY → Phase 1 audit (279th, read-only flat verification).

**F002 status**: `gh issue create` → GraphQL "Resource not accessible by integration" — **403 CONCLUSIVE (81st)** — token lacks `issues: write`. GitHub-native issue creation remains blocked. All findings recorded here in the ledger, the established output channel.

## Finding status table (279th basis — active tracked findings)

| Finding | Category | Priority | Status | Observation |
| ------- | -------- | -------- | ------ | ----------- |
| F002    | ci       | P1       | HELD   | Token lacks `issues: write` — **403 CONCLUSIVE (81st)** (re-probed direct this run) |
| F004    | security | P2       | HELD   | **57 refs / 10 unique names** (workflows `*.yml` only) — byte-identical to 278th basis, **zero growth** |
| F005    | docs     | P3       | HELD   | **232 files** (127th obs): 231 ledger + SECURITY_AUDIT_NOTE.md — **zero source files** (F251 verified holding, 30th clean obs); ledger-only drift by convention (grew by 3 from the 278th ledger merged in #840) |
...
| F037    | security | P1       | HELD   | 12 workflow violations (2 CRITICAL + 10 HIGH), **180th obs**, push-blocked F050 (56th documented pass) |
| F038    | ci       | P1       | HELD   | orchestrator workflow **10/10 most recent FAILED** (~70+ days, latest 2026-08-20T00:50:16Z) |
| F050    | ci       | P1       | HELD   | token lacks `workflows: write` — workflow-file changes blocked (56th documented pass) |
| F063    | ci       | P1       | HELD (improving) | on-pull **29/29 completed successes** visible, 0 failures; **streak ≥24 maintained** |
| F064    | ci       | P3       | HELD   | `.nvmrc` 22 vs runtime node v20.20.2 vs CI node 20 vs devcontainer node 20; lint-staged EBADENGINE (wants node ≥22.22.1); **pytest env-parity UNAVAILABLE this run (25th obs — continued unavailable)** |
...
| F228    | docs     | P2       | RESOLVED maintained | README.md:312 12-violation note / SECURITY.md:46-50 / deployment.md accurate at HEAD **`65f0142`** (**41st clean obs**) |
| F229    | security | P1       | HELD   | commit `0056ad8` still UNREACHABLE (**44th obs**); unhardened `curl -fsSL https://opencode.ai/install \| bash` (no pipefail/retry) in **on-pull.yml:63 AND on-push.yml:63** |
| F230    | docs     | P3       | RESOLVED maintained | `prettier --check docs/blueprint.md docs/task.md` clean (**43rd clean obs**) |
| F231    | bug      | P2       | RESOLVED maintained | monitorBuild report-after-stop verified at source (build-performance.js:216/268/296) — **33rd clean obs** |
| F232    | bug      | P1       | RESOLVED maintained | manifest hash includes lat/lon (manifest.js:135-136) — **33rd clean obs** |
...
| F234    | security | P1       | RESOLVED maintained | EXTERNAL_DATA_DIR injection closed (fetch-data.js:183-198) — **33rd clean obs** |
...
| F239    | docs     | P2       | RESOLVED maintained | api.md:3888 Comparison Module + api.md:6627 Test Helpers Module present (**29th obs**) |
...
| F251    | chore    | P3       | RESOLVED maintained | `scripts/data-schema.test.js` prettier-clean (**30th clean obs** — zero source drift invariant) |
...
## Suggested resolutions for open HELD findings (unchanged from 238th batch, held)
...
- `docs/issues/2026-08-20/51-audit-report-2026-08-20-279th.md`
- `docs/issues/2026-08-20/52-issue-records-239th-batch-delta-279th.md` (this file)
- `docs/issues/2026-08-20/53-phase2-3-decision-279th.md`