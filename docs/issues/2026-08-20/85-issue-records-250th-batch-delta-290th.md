# Issue Records — 250th Batch Delta (290th run, 2026-08-20)

**Ledger**: 250th batch delta. **Trigger**: Phase 0 probe → **1 open PR (#851, 289th ledger)** → PR HANDLER MODE → merged → re-probe **0 open PRs / 0 open issues** → Phase 0.3 EMPTY → Phase 1 audit (290th, read-only flat verification).

**F002 status**: `gh issue create` → GraphQL "Resource not accessible by integration (createIssue)" — **403 CONCLUSIVE (92nd)** — token lacks `issues: write`. GitHub-native issue creation remains blocked. All findings recorded here in the ledger, the established output channel.

## Finding status table (290th basis — active tracked findings)

| Finding | Category | Priority | Status              | Observation                                                                                                                                                                                                                                           |
| ------- | -------- | -------- | ------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| F002    | ci       | P1       | HELD                | Token lacks `issues: write` — **403 CONCLUSIVE (92nd)** (re-probed direct this run)                                                                                                                                                                   |
| F004    | security | P2       | HELD                | **57 refs / 10 unique names** (workflows `*.yml` only) — byte-identical to 289th basis, **zero growth**                                                                                                                                               |
| F005    | docs     | P3       | HELD                | **246 files** (136th obs): 245 ledger + SECURITY_AUDIT_NOTE.md — **zero source files** (F251 verified holding, 41st clean obs); ledger-only drift by convention (246 at post-#851 HEAD; this run's files 84/85/86 written prettier-clean → no growth) |
...
| F037    | security | P1       | HELD                | 12 workflow violations (2 CRITICAL + 10 HIGH), **191st obs**, push-blocked F050 (67th documented pass)                                                                                                                                                |
| F063    | ci       | P3       | HELD                | on-pull streak maintained (≥24 threshold; last 29 completed schedule runs all success + 1 in-progress at probe) — 39th obs                                                                                                                            |
| F064    | test     | P3       | HELD                | pytest env parity UNAVAILABLE — 36th obs (python3 -m pytest → "No module named pytest")                                                                                                                                                               |
| F225    | refactor | P3       | HELD                | 4 import edges src/services/BuildOrchestrator.js:52-55 → ../../scripts/*                                                                                                                                                                              |
| F228    | docs     | P3       | HELD                | README.md:312 12-violation note accurate at HEAD `189f692`                                                                                                                                                                                            |
| F229    | security | P2       | HELD                | **unhardened `curl                                                                                                                                                                                                                                    | bash` installer in BOTH on-pull.yml:63 AND on-push.yml:63** — 55th obs |
| F230    | docs     | P3       | HELD                | blueprint.md/task.md prettier-clean — 54th clean obs                                                                                                                                                                                                  |
...
| F239    | docs     | P3       | HELD                | api.md:3888 Comparison Module + api.md:6627 Test Helpers Module present — 40th obs                                                                                                                                                                    |
...
| F251    | test     | P3       | HELD                | scripts/data-schema.test.js prettier-clean — 41st clean obs                                                                                                                                                                                           |

## Held-finding detail (290th basis)

### F037 — workflow security violations (191st obs, push-blocked F050 67th documented pass)
