# Issue Records — 243rd Batch Delta (283rd run, 2026-08-20)

**Ledger**: 243rd batch delta. **Trigger**: Phase 0 → 1 open PR (#844, 282nd ledger) → PR HANDLER MODE: **merged #844** → Phase 0 re-probe 0 open PRs / 0 open issues → Phase 0.3 EMPTY → Phase 1 audit (283rd, read-only flat verification).

**F002 status**: `gh issue create` → GraphQL "Resource not accessible by integration (createIssue)" — **403 CONCLUSIVE (85th)** — token lacks `issues: write`. GitHub-native issue creation remains blocked. All findings recorded here in the ledger, the established output channel.

## Finding status table (283rd basis — active tracked findings)

| Finding | Category | Priority | Status | Observation |
| ------- | -------- | -------- | ------ | ----------- |
| F002    | ci       | P1       | HELD   | Token lacks `issues: write` — **403 CONCLUSIVE (85th)** (re-probed direct this run) |
| F004    | security | P2       | HELD   | **57 refs / 10 unique names** (workflows `*.yml` only) — byte-identical to 282nd basis, **zero growth** |
| F005    | docs     | P3       | HELD   | **243 files** (131st obs): 242 ledger + SECURITY_AUDIT_NOTE.md — **zero source files** (F251 verified holding, 34th clean obs); ledger-only drift by convention (243 at post-#844 HEAD; +3 from this run's own ledger files 63/64/65 → 246) |
...
| F037    | security | P1       | HELD   | 12 workflow violations (2 CRITICAL + 10 HIGH), **184th obs**, push-blocked F050 (60th documented pass) |
| F063    | ci       | P3       | HELD   | on-pull streak maintained (≥24 threshold; last 5 completed schedule runs all success) — 32nd obs |
| F064    | test     | P3       | HELD   | pytest env parity UNAVAILABLE — 29th obs |
| F225    | refactor | P3       | HELD   | 4 import edges src/services/BuildOrchestrator.js → ../../scripts/* |
| F228    | docs     | P3       | HELD   | README.md:312 12-violation note accurate at HEAD `1216aa9` |
| F229    | security | P2       | HELD   | `0056ad8` unreachable; **unhardened `curl|bash` installer in BOTH on-pull.yml:63 AND on-push.yml:63** — 48th obs |
| F230    | docs     | P3       | HELD   | blueprint.md/task.md prettier-clean — 47th clean obs |
...
| F239    | docs     | P3       | HELD   | api.md:3888 Comparison Module + api.md:6627 Test Helpers Module present — 33rd obs |
...
| F251    | test     | P3       | HELD   | data-schema.test.js prettier-clean — 34th clean obs |

## Held-finding detail (283rd basis)

### F037 — workflow security violations (184th obs, push-blocked F050 60th documented pass)
...
### F005 — prettier failure set (243 files, zero source)

`npx prettier --check "docs/**/*.md"` → 243 ledger/convention files fail; **0 source files** in the set (verified by extension filter). Ledger-only drift by convention; grows per merged ledger run.
