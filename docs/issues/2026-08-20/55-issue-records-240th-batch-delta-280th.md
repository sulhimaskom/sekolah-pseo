# Issue Records — 240th Batch Delta (280th run, 2026-08-20)

**Ledger**: 240th batch delta. **Trigger**: Phase 0 → 1 open PR (#841, 279th ledger) → PR HANDLER MODE: **merged #841** → Phase 0 re-probe 0 open PRs / 0 open issues → Phase 0.3 EMPTY → Phase 1 audit (280th, read-only flat verification).

**F002 status**: `gh issue create` → GraphQL "Resource not accessible by integration" — **403 CONCLUSIVE (82nd)** — token lacks `issues: write`. GitHub-native issue creation remains blocked. All findings recorded here in the ledger, the established output channel.

## Finding status table (280th basis — active tracked findings)

| Finding | Category | Priority | Status | Observation |
| ------- | -------- | -------- | ------ | ----------- |
| F002    | ci       | P1       | HELD   | Token lacks `issues: write` — **403 CONCLUSIVE (82nd)** (re-probed direct this run) |
| F004    | security | P2       | HELD   | **57 refs / 10 unique names** (workflows `*.yml` only) — byte-identical to 279th basis, **zero growth** |
| F005    | docs     | P3       | HELD   | **235 files** (128th obs): 234 ledger + SECURITY_AUDIT_NOTE.md — **zero source files** (F251 verified holding, 31st clean obs); ledger-only drift by convention (grew by 3 from the 279th ledger merged in #841) |
...
| F037    | security | P1       | HELD   | 12 workflow violations (2 CRITICAL + 10 HIGH), **181st obs**, push-blocked F050 (57th documented pass) |
| F038    | ci       | P1       | HELD   | orchestrator workflow **10/10 most recent FAILED** (~70+ days, latest 2026-08-20T00:50:16Z) |
| F050    | ci       | P1       | HELD   | token lacks `workflows: write` — workflow-file changes blocked (57th documented pass) |
| F063    | ci       | P1       | HELD (improving) | on-pull **29/29 completed successes** visible, 0 failures; **streak ≥24 maintained** |
| F064    | ci       | P3       | HELD   | `.nvmrc` 22 vs runtime node v20.20.2 vs CI node 20 vs devcontainer node 20; lint-staged EBADENGINE (wants node ≥22.22.1); **pytest env-parity UNAVAILABLE this run (26th obs — continued unavailable)** |
...
| F228    | docs     | P2       | RESOLVED maintained | README.md:312 12-violation note / SECURITY.md:46-50 / deployment.md accurate at HEAD **`93316b6`** (**42nd clean obs**) |
| F229    | security | P1       | HELD   | commit `0056ad8` still UNREACHABLE (**45th obs**); unhardened `curl -fsSL https://opencode.ai/install \| bash` (no pipefail/retry) in **on-pull.yml:63 AND on-push.yml:63** |
| F230    | docs     | P3       | RESOLVED maintained | `prettier --check docs/blueprint.md docs/task.md` clean (**44th clean obs**) |
| F231    | bug      | P2       | RESOLVED maintained | monitorBuild report-after-stop verified at source (build-performance.js:216/268/296) — **34th clean obs** |
| F232    | bug      | P1       | RESOLVED maintained | manifest hash includes lat/lon (manifest.js:135-136) — **34th clean obs** |
...
| F234    | security | P1       | RESOLVED maintained | EXTERNAL_DATA_DIR injection closed (fetch-data.js:183-198/249) — **34th clean obs** |
...
| F239    | docs     | P2       | RESOLVED maintained | api.md:3888 Comparison Module + api.md:6627 Test Helpers Module present (**30th obs**) |
...
| F247    | bug      | P3       | RESOLVED maintained | no `/provinsi/undefined/` path at source — verified holding (grep hits confined to homepage.test.js:710/712 assertions) |
...
| F251    | chore    | P3       | RESOLVED maintained | `scripts/data-schema.test.js` prettier-clean (**31st clean obs** — zero source drift invariant) |
...
## Suggested resolutions for open HELD findings (unchanged from 239th batch, held)
...
- `docs/issues/2026-08-20/54-audit-report-2026-08-20-280th.md`
- `docs/issues/2026-08-20/55-issue-records-240th-batch-delta-280th.md` (this file)
- `docs/issues/2026-08-20/56-phase2-3-decision-280th.md`