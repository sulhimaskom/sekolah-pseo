# Issue Records — 246th Batch Delta (286th run, 2026-08-20)

**Ledger**: 246th batch delta. **Trigger**: Phase 0 probe → **1 open PR (#847, 285th ledger)** → PR HANDLER MODE → merged → re-probe **0 open PRs / 0 open issues** → Phase 0.3 EMPTY → Phase 1 audit (286th, read-only flat verification).

**F002 status**: `gh issue create` → GraphQL "Resource not accessible by integration (createIssue)" — **403 CONCLUSIVE (88th)** — token lacks `issues: write`. GitHub-native issue creation remains blocked. All findings recorded here in the ledger, the established output channel.

## Finding status table (286th basis — active tracked findings)

| Finding | Category | Priority | Status              | Observation                                                                                                                                                                                                                                           |
| ------- | -------- | -------- | ------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| F002    | ci       | P1       | HELD                | Token lacks `issues: write` — **403 CONCLUSIVE (88th)** (re-probed direct this run)                                                                                                                                                                   |
| F004    | security | P2       | HELD                | **57 refs / 10 unique names** (workflows `*.yml` only) — byte-identical to 285th basis, **zero growth**                                                                                                                                               |
| F005    | docs     | P3       | HELD                | **246 files** (133rd obs): 245 ledger + SECURITY_AUDIT_NOTE.md — **zero source files** (F251 verified holding, 37th clean obs); ledger-only drift by convention (246 at post-#847 HEAD; this run's files 72/73/74 written prettier-clean → no growth) |
...
| F037    | security | P1       | HELD                | 12 workflow violations (2 CRITICAL + 10 HIGH), **187th obs**, push-blocked F050 (63rd documented pass)                                                                                                                                                |
| F063    | ci       | P3       | HELD                | on-pull streak maintained (≥24 threshold; last 29 completed schedule runs all success + 1 in-progress at probe) — 35th obs                                                                                                                            |
| F064    | test     | P3       | HELD                | pytest env parity UNAVAILABLE — 32nd obs (python3 -m pytest → "No module named pytest")                                                                                                                                                               |
| F225    | refactor | P3       | HELD                | 4 import edges src/services/BuildOrchestrator.js:52-55 → ../../scripts/*                                                                                                                                                                              |
| F228    | docs     | P3       | HELD                | README.md:312 12-violation note accurate at HEAD `ce743aa`                                                                                                                                                                                            |
| F229    | security | P2       | HELD                | **unhardened `curl                                                                                                                                                                                                                                    | bash` installer in BOTH on-pull.yml:63 AND on-push.yml:63** — 51st obs |
| F230    | docs     | P3       | HELD                | blueprint.md/task.md prettier-clean — 50th clean obs                                                                                                                                                                                                  |
...
| F239    | docs     | P3       | HELD                | api.md:3888 Comparison Module + api.md:6627 Test Helpers Module present — 36th obs                                                                                                                                                                    |
...
| F251    | test     | P3       | HELD                | scripts/data-schema.test.js prettier-clean — 37th clean obs                                                                                                                                                                                           |

## Held-finding detail (286th basis)

### F037 — workflow security violations (187th obs, push-blocked F050 63rd documented pass)
...
`npx prettier --check "docs/**/*.md"` → 246 ledger/convention files fail; **0 source files** in the set (verified by extension filter). Ledger-only drift by convention; this run's ledger files are written prettier-clean, so the count does not grow.
