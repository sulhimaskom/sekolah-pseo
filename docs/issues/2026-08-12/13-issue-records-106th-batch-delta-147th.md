# Issue Records — 106th Batch (Delta, 147th verification, 2026-08-12)

**Evaluation Date**: 2026-08-12
**Evaluator**: Sisyphus (ULW Loop, autonomous run)
**Basis**: 146th audit matrix on `4a65c8f`, re-executed fresh this run (see
`12-audit-report-2026-08-12-147th.md`).
**Convention**: GitHub issue creation is denied for this integration token (F002 —
`GraphQL: Resource not accessible by integration (createIssue)`, 135th consecutive
denial). Per the 146-run docs-only convention, findings are recorded in this ledger.
...
made — the worktree is clean on main (`4a65c8f`).

## Stability confirmation (held findings)

| ID   | Category/Priority | Status       | Evidence this run                                                      |
| ---- | ----------------- | ------------ | ---------------------------------------------------------------------- |
| F005 | docs/P2           | HELD         | prettier: 88 ledger files — **stable 6th run** (0 source files)        |
| F037 | security/P0       | HELD         | duplicate `GEMINI_API_KEY` API_KEY refs — **CRITICAL** (48th obs)      |
| F038 | security/P0       | HELD         | workflow `id-token`/`actions` write on non-merge, GH_TOKEN refs (48th) |
| F063 | ci/P1             | HELD         | PR-event workflow runs land in `action_required`; schedule runs pass   |
| F002 | ci/P1             | HELD         | `gh issue create` denied (135th)                                       |
| F018 | feature/P1        | HELD         | data STALE 23 days (threshold 7)                                       |
| F025 | feature/P1        | HELD         | Pages root HTTP 404 (curl 404 measured in-session)                     |
| F064 | chore/P2          | HELD         | .nvmrc 22 vs runtime v20.20.2 vs CI node 20                            |
| F004 | security/P2       | HELD         | 57 `secrets.*` refs / 10 unique names (5th stable)                     |
...
| F014 | test/P1           | NOT OBSERVED | 4th consecutive clean run — 1104/1100/0/4-skip                         |
...
### R-147-1: F004 count stable at 57 refs / 10 unique names (5th consecutive)

`grep -rhEo "secrets\.[A-Z_]+" .github/workflows/*.yml` yields **57 refs / 10
unique names** — identical distribution to the 143rd–146th runs (GITHUB_TOKEN 14,
...
SUPABASE_ANON_KEY 1). Workflows byte-identical; grep-method stable for 5 runs.
Monitoring continues.

### R-147-2: F014 race probe — 4th consecutive clean run
...
### R-147-3: Source re-verification — maintained resolutions intact
...
F050/F063/F064/F065). F014 has now been clean for 4 consecutive runs (monitoring
