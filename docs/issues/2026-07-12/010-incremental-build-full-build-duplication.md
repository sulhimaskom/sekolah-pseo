...
- `scripts/build-pages.js`: ~200 lines of shared setup followed by separate full-build path (~150 lines) and incremental-build path (~100 lines)
...
3. Reduce build-pages.js controller from 555 to < 300 lines by extracting the pipeline into a service
