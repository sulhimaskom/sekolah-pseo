# Phase 1 — Diagnostic & Comprehensive Scoring Report (68th verification, 2026-08-07)

**Evaluation Date**: 2026-08-07
**Evaluator**: Sisyphus (ULW Loop, autonomous run)
**Repository**: sulhimaskom/sekolah-pseo
**Default Branch**: main (`bb49915` — 67th run docs, PR #599 merged) [detected: main]
...
Composite **71.0 (±0.0)** — third consecutive flat confirmation run. HEAD ==
origin/main == `bb49915`; zero code churn since the 67th run. The full command
...
- **F002 (issue-creation block)** — CONFIRMED 65th consecutive: `gh issue
create` → `GraphQL: Resource not accessible by integration (createIssue)`.
  Root cause confirmed at source: `on-pull.yml:9-14` `permissions:` block omits
...
- **F028 (npm audit HIGH)** — **RESOLVED this run**: `npm audit` now reports
  **0 vulnerabilities** (exit 0). The 67th-run ledger recorded 1 HIGH
  (brace-expansion@5.0.8); the transitive fix is now present in the resolved
  tree. First positive delta in the audit matrix since the 66th run.
...
- **F024 (build omits sitemap)** — CONFIRMED: after `npm run build` the dist/
  tree has no `sitemap-index.xml` (`ls dist/*.xml` → no such file); build only
  runs build-pages.js, sitemap is a separate `npm run sitemap` step.
- **Workflow security cluster (F037/F038 CRITICAL + F056-F059 + F013 +
  F063/F064/F065)** — CONFIRMED: `node scripts/check-workflow-security.js`
  exits **1 with 12 violations (2 CRITICAL DUPLICATE_API_KEY + 10 HIGH)** across
  on-push.yml / parallel.yml / orchestrator.yml / architect-agent.yml.
- **F062 (docs drift)** — CONFIRMED: `docs/release.md` documents a phantom
  `.github/workflows/release.yml` (no such file); `docs/api.md:554-575`
  documents a phantom `addNumbers(a, b)` function not present in source.
- **F065 (dead pre-commit gate)** — CONFIRMED: `.husky/pre-commit` line 3 wraps
  the workflow-security checker in `|| echo skipped`, so a failure-free local
  commit can ship security regressions.
...
git-history forensics, direct source reads).
...
| Command                                   | Result                                                                                                     |
| ----------------------------------------- | ---------------------------------------------------------------------------------------------------------- |
| `git fetch` + window check                | HEAD == origin/main == `bb49915` — zero churn since 67th                                                   |
| `npm install`                             | 131 packages added; `EBADENGINE` warning lint-staged@17.3.0 needs node >=22.22.1, env v20.20.2 (F012/F064) |
| `npm audit`                               | **0 vulnerabilities** — F028 RESOLVED (67th ledger: 1 HIGH brace-expansion@5.0.8)                          |
| `npm run lint`                            | exit 0 — zero ESLint errors/warnings                                                                       |
| `npm run format:check`                    | **exit 1 — 64 files** (F005 HELD, no growth)                                                               |
| `npm run build`                           | exit 0 — homepage + 2 province + 2 school + robots.txt + styles.css; 28 ms; budgets met                    |
...
| `ls dist/*.xml` post-build                | no `sitemap-index.xml` → **F024 confirmed**                                                                |
| `gh issue create` probe                   | **403 `GraphQL: Resource not accessible by integration (createIssue)` (F002, 65th consecutive)**           |
...
- **Observations**: All 10 CLI entrypoints end-to-end correct; 1060 JS + 27 Py tests pass; coverage ~95%; build deterministic (28 ms, budgets met).
- **Evidence**: `scripts/build-pages.js`, `src/services/BuildOrchestrator.js`; test run logs (exit 0); coverage report.
- **Impact/Risk**: The main residual correctness risk is untrusted-input handling on the boundary (F056 path-guard bypass, F057 prototype-key injection via NPSN on plain maps) in `scripts/config.js`, `scripts/manifest.js`, `src/services/BuildOrchestrator.js`.
...
- **Observations**: strong — 1060 JS / 27 py, coverage 94.94/92.2/96.65, co-located `node:test` files, c8 gates.
- **Evidence**: test matrix results; package.json scripts.
...
- **Observations**: single runtime dep (`pino`); CommonJS only; eslint security rules active; **npm audit now clean (0 vulnerabilities — F028 RESOLVED)**. `EBADENGINE` mismatch (env node 20 vs lint-staged >=22.22.1) and npm strict-mode absent.
- **Evidence**: package.json (`"dependencies": {"pino": "^10.3.1"}`); install/audit logs.
- **Score rationale**: −16 (F012, F064). The F028 HIGH is removed from the ledger this run; no score change because the deduction was attributable to F012/F064, which persist.
...
- **Observations**: 31-76 pages/sec (this run 71.43); budgets met; caching (WeakMap, memoized CSS, cached escape/slug).
...
- **Observations**: repo's own `check-workflow-security.js` **exits 1 with 12 violations**: 2 CRITICAL `DUPLICATE_API_KEY` (API_KEY=GEMINI, on-push.yml:26, parallel.yml:37/282/362/416), 4 `id-token: write` non-OIDC, 4 `actions: write` non-merge, 2 `GH_TOKEN` (orchestrator.yml:33/41, architect-agent.yml:37). Plus client-side unhandled rejection and the boundary findings (F056/F057).
- **Evidence**: workflow-security CLI output (verified firsthand); source files; SECURITY_AUDIT_NOTE.md stale claim.
- **Impact/Risk**: high; a failure-free local commit can still ship these regressions because `.husky/pre-commit` wraps the checker in `|| echo skipped` (verified at `.husky/pre-commit:3`) — the gate is dead.
...
Local Dev Setup (DX) (W12, 82) — npm scripts clean; but `.env.example` documents 7 vars while CI injects 10+ secrets; dotenv added but not consumed; `manifest` doc mismatch (`.build-manifest.json`). −18 (F061/F012/F062).
Documentation Accuracy (W14, 44) — **F062 drift cluster held**: release.md:67 references non-existent `.github/workflows/release.yml` (verified: no such file); api.md phantom `addNumbers` (verified at api.md:554-575); setup.md v20-claim vs .nvmrc=22; deployment.md dist tree/schools.json mismatch.
Debuggability (DX) (W10, 78) — `--json` modes; pino structured. −7.
...
- **Observations**: `on-push.yml` (README claims "quality gate (lint + format check)") — **false**; grep across `.github/workflows` for `npm run build|test|lint|format|pytest` returns zero matches; `on-pull.yml` / `on-push.yml` / `parallel.yml` run `opencode run` (agent) only; `parallel.yml` runs `npm install || true` (swallows failure) and `continue-on-error`/fail-fast on the specialists job.
- **Evidence**: grep result, `on-push.yml`, `on-pull.yml:44/51`, `parallel.yml:72/227/230/347`.
- **Impact/Risk**: a red merge with zero gates is the norm; regressions routinely slip silently.
- **Score rationale**: −44.

### Release & Rollback Safety (W20, score 44, weighted 8.80)

- **Observations**: no `release.yml` exists (only orchestrator/parallel/on-pull/on-push/opencode/architect), no tag-version workflow; `docs/release.md:67` documents a phantom tag→release flow (verified: no release.yml in `.github/workflows`); `package.json` still 1.0.0.
...
- **Observations**: `.env.example` lists 7 local vars; CI injects 10+ secrets not in .env.example; node version mismatch (.nvmrc=22 vs workflows node-version 20); pre-commit-config (pre-commit framework) not actually installed (requirements.txt lacks it); `setup-opencode` composite action unused/dead.
...
- **Observations**: 60+ tracked findings across 27-date ledger; filter severity cluster includes a 12-issue workflow matrix as a huge audit debt (F005, F037/F038, F056-F059, F062, F063/F064/F065).
...
- **Evidence**: commit log cadence (68 verification runs).
...
create` is blocked (403 `createIssue`, F002 held 65 sessions), findings ship as
...
- Workflow-security cluster F037/F038 + F056-F059 (`security` / `P1`).
- F065 (`ci` / `P2`) — Continue-on-error / `|| echo skipped` swallow CI failures.
- F064 (`ci` / `P2`) — Dependabot red-merge + node-engine mismatch.
- F061/F062 (`chore` / `P2`, `docs` / `P3`) — .env/doc drift.
- F028 (`security` / `P2`) — **RESOLVED this run** (npm audit 0 vulnerabilities).
...
| ----------------- | ----------------------- | ---------------------------------------------------------- | --------------------------------------- |
| 2026-08-07 ~17:42 | Phase-0 gate            | gh.open: pr/issues                                         | 0 PR / 0 issues → Phase 1               |
| ~17:44            | install deps            | `npm install`, `pip -r requirements`                       | 131 pkgs; pytest env; audit 0 vulns     |
| ~17:45            | command matrix          | lint/format/build/test/coverage/sitemap/validate/freshness | see table                               |
| ~17:45            | security gate           | `scripts/check-workflow-security.js`                       | exit 1, 12 violations                   |
| ~17:46            | issue-capability probe  | `gh issue create`                                          | 403 createIssue (F002 confirmed)        |
| ~17:46            | source verification     | on-pull.yml perms / pre-commit / release.yml / api.md      | F002/F065/F062 confirmed, F028 resolved |
| ~17:47            | record audit + findings | `docs/issues/2026-08-07/39-*.md`                           | this file                               |
...
- **Final status**: **waiting for human review** — the Phase 2 (hardening/repair) or Phase 3 (strategy) next step requires a token with `issues:write` + `workflows:write` to unblock the pipeline and meaningful change-velocity; meanwhile the sole concrete remediation (a `ci.yml` `npm test`-gate + removal of the 12 workflow-security violations) is deferred to review.
...
1. **Flat confirm (71.0, ±0)** for the 3rd consecutive run — zero code churn; the sole matrix delta is F028 RESOLVED (npm audit 0 vulnerabilities).
2. **F033 remains over the fence**: the virtual CI surface is unstable as-is; every fix must first pass the blocked-by-F002 token-gate review.
