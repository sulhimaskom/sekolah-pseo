# Issue Records — 134th Batch (Delta, 175th verification, 2026-08-14)

**Evaluation Date**: 2026-08-14
**Evaluator**: Sisyphus (ULW Loop, autonomous run)
**Basis**: 174th audit matrix on `7ceb39f`, re-executed fresh this run (see
`59-audit-report-2026-08-14-175th.md`).
**Convention**: GitHub issue creation is denied for this integration token (F002 —
`GraphQL: Resource not accessible by integration (createIssue)`, **163rd
consecutive denial**, freshly probed this run). Per the 174-run docs-only
convention, findings are recorded in this ledger. Each entry carries the mandated
category + priority labels (contract §4).

## This run: PR #715 merged (PR HANDLER MODE), orchestrator 10/10 HELD (23 days), pull CI best held 14/1/0 (24th window), F005 flat 96 (23rd), F004 stable 59/10, F018 held at 25 days

## Open findings (held)

| ID   | Category    | Priority | Title                                                                         | State           |
| ---- | ----------- | -------- | ----------------------------------------------------------------------------- | --------------- |
| F037 | security    | P0       | Workflow security: 12 violations (2 CRITICAL DUPLICATE_API_KEY + 10 HIGH)     | HELD (76th obs) |
| F038 | ci          | P1       | Orchestrator workflow fails 10/10 (secrets.GH_TOKEN vs GITHUB_TOKEN, 23 days) | HELD (23 days)  |
| F002 | chore       | P1       | GitHub issue creation denied for integration token (163rd denial)             | HELD            |
| F044 | security    | P2       | Over-scoped secrets: 10 unique names / 59 refs across workflows + template    | HELD            |
| F005 | chore       | P2       | 96 docs/issues ledger files fail `prettier --check` (23rd flat)               | HELD/FLAT       |
...
| F063 | ci          | P1       | pull CI hourly timeout cluster (08-12) — SELF-RECOVERED, 24th clean window    | HEALTHY         |
