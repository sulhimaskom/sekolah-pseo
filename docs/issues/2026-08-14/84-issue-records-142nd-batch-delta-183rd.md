# Issue Records — 142nd Batch (Delta, 183rd verification, 2026-08-14)

**Evaluation Date**: 2026-08-14
**Evaluator**: Sisyphus (ULW Loop, autonomous run)
**Basis**: 182nd audit matrix on `bc60fec`, re-executed fresh this run (see
`83-audit-report-2026-08-14-183rd.md`).
**Convention**: GitHub issue creation is denied for this integration token (F002 —
`GraphQL: Resource not accessible by integration (createIssue)`, **171st
consecutive denial**, freshly probed this run). Per the 182-run docs-only
convention, findings are recorded in this ledger. Each entry carries the mandated
category + priority labels (contract §4).

## This run: PR #723 merged (PR HANDLER MODE, prettier-clean at gate), orchestrator 10/10 HELD (26 days), pull CI best held 14/0/0 (32nd window), F005 flat 99 (31st), F004 stable 59/10, F018 held at 25 days

## Open findings (held)

| ID   | Category    | Priority | Title                                                                         | State           |
| ---- | ----------- | -------- | ----------------------------------------------------------------------------- | --------------- |
| F037 | security    | P0       | Workflow security: 12 violations (2 CRITICAL DUPLICATE_API_KEY + 10 HIGH)     | HELD (84th obs) |
| F038 | ci          | P1       | Orchestrator workflow fails 10/10 (secrets.GH_TOKEN vs GITHUB_TOKEN, 26 days) | HELD (26 days)  |
| F002 | chore       | P1       | GitHub issue creation denied for integration token (171st denial)             | HELD            |
| F044 | security    | P2       | Over-scoped secrets: 10 unique names / 59 refs across workflows + template    | HELD            |
| F005 | chore       | P2       | 99 docs/issues ledger files fail `prettier --check` (31st flat, held)         | HELD/FLAT       |
...
| F063 | ci          | P1       | pull CI hourly timeout cluster (08-12) — SELF-RECOVERED, 32nd clean window    | HEALTHY         |
