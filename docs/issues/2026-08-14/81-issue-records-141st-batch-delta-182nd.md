# Issue Records — 141st Batch (Delta, 182nd verification, 2026-08-14)

**Evaluation Date**: 2026-08-14
**Evaluator**: Sisyphus (ULW Loop, autonomous run)
**Basis**: 181st audit matrix on `2449dc7`, re-executed fresh this run (see
`80-audit-report-2026-08-14-182nd.md`).
**Convention**: GitHub issue creation is denied for this integration token (F002 —
`GraphQL: Resource not accessible by integration (createIssue)`, **170th
consecutive denial**, freshly probed this run). Per the 181-run docs-only
convention, findings are recorded in this ledger. Each entry carries the mandated
category + priority labels (contract §4).

## This run: PR #722 merged (PR HANDLER MODE, prettier gate fix on file 77), orchestrator 10/10 HELD (25 days), pull CI best held 14/0/0 (31st window), F005 flat 99 (30th), F004 stable 59/10, F018 held at 25 days

## Open findings (held)

| ID   | Category    | Priority | Title                                                                         | State           |
| ---- | ----------- | -------- | ----------------------------------------------------------------------------- | --------------- |
| F037 | security    | P0       | Workflow security: 12 violations (2 CRITICAL DUPLICATE_API_KEY + 10 HIGH)     | HELD (83rd obs) |
| F038 | ci          | P1       | Orchestrator workflow fails 10/10 (secrets.GH_TOKEN vs GITHUB_TOKEN, 25 days) | HELD (25 days)  |
| F002 | chore       | P1       | GitHub issue creation denied for integration token (170th denial)             | HELD            |
| F044 | security    | P2       | Over-scoped secrets: 10 unique names / 59 refs across workflows + template    | HELD            |
| F005 | chore       | P2       | 99 docs/issues ledger files fail `prettier --check` (30th flat, held)         | HELD/FLAT       |
...
| F063 | ci          | P1       | pull CI hourly timeout cluster (08-12) — SELF-RECOVERED, 31st clean window    | HEALTHY         |
