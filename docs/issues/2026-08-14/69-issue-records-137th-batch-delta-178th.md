# Issue Records — 137th Batch (Delta, 178th verification, 2026-08-14)

**Evaluation Date**: 2026-08-14
**Evaluator**: Sisyphus (ULW Loop, autonomous run)
**Basis**: 177th audit matrix on `590e858`, re-executed fresh this run (see
`68-audit-report-2026-08-14-178th.md`).
**Convention**: GitHub issue creation is denied for this integration token (F002 —
`GraphQL: Resource not accessible by integration (createIssue)`, **166th
consecutive denial**, freshly probed this run). Per the 177-run docs-only
convention, findings are recorded in this ledger. Each entry carries the mandated
category + priority labels (contract §4).

## This run: PR #718 merged (PR HANDLER MODE), orchestrator 10/10 HELD (24 days), pull CI best held 14/0/0 (27th window), F005 flat 96 (26th), F004 stable 59/10, F018 held at 25 days

## Open findings (held)

| ID   | Category    | Priority | Title                                                                         | State           |
| ---- | ----------- | -------- | ----------------------------------------------------------------------------- | --------------- |
| F037 | security    | P0       | Workflow security: 12 violations (2 CRITICAL DUPLICATE_API_KEY + 10 HIGH)     | HELD (79th obs) |
| F038 | ci          | P1       | Orchestrator workflow fails 10/10 (secrets.GH_TOKEN vs GITHUB_TOKEN, 24 days) | HELD (24 days)  |
| F002 | chore       | P1       | GitHub issue creation denied for integration token (166th denial)             | HELD            |
| F044 | security    | P2       | Over-scoped secrets: 10 unique names / 59 refs across workflows + template    | HELD            |
| F005 | chore       | P2       | 96 docs/issues ledger files fail `prettier --check` (26th flat)               | HELD/FLAT       |
...
| F063 | ci          | P1       | pull CI hourly timeout cluster (08-12) — SELF-RECOVERED, 27th clean window    | HEALTHY         |
