# Issue Records — 81st verification (labeled findings; GitHub-issue output blocked by F002)
...
`createIssue` (F002 held 77 sessions; re-verified this run) — these records are the repo's
...
## Issue 3 — F002 no `issues: write` permission (77th consecutive blocked)
...
(createIssue)` — 77 consecutive runs. `gh pr create`/`gh pr merge` work.
...
- **Criteria breakdown**:
  - F018: `node scripts/check-freshness.js` → `STALE` — last update 2026-07-20 (**19 days**,
    ...
    `fetch-data.js` `findCsvFiles()` requires `.csv` → refresh would fail. Verified this run via
    temp clone.
...
## Issue 5 — F014 parallel flake: clean 7th consecutive run (latent, P2)

- **Category/priority**: `test` / `P2`
- **Evaluation date**: 2026-08-08
- **Domain**: A. Testability 74; Determinism 78
- **Criteria breakdown**: fs/tmp race absent 7 consecutive verifications; suite
...
- **Evidence**: `npm run build` → `dist/sitemap-index.xml` + `dist/sitemap-001.xml` present
  (28ms, budgets met); 6/6 across last runs.
- **Files**: `scripts/build-pages.js`, `scripts/sitemap.js`
- **Status**: maintained RESOLVED.

## Issue 7 — F028 `npm audit` clean — 14th consecutive (maintained RESOLVED, P2)
...
## Issue 9 — F017 phantom `addNumbers` API docs (FIXED this run, P3)

- **Category/priority**: `docs` / `P3`
- **Evaluation date**: 2026-08-08
- **Domain**: C. Documentation Accuracy (47); A. Readability
- **Criteria breakdown**: `docs/api.md` documented `addNumbers(a, b)` (554–577) but no such
  export exists in `scripts/utils.js` (verified 0 matches in src/scripts). Localized phantom
  entry alongside real `escapeHtml`/`walkDirectory` sections.
- **Fix**: phantom block removed from `docs/api.md` this run (atomic, non-cosmetic);
  `grep addNumbers` → 0. Full suite re-run green.
- **Files**: `docs/api.md`
