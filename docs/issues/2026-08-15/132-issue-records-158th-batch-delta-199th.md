# Issue Records — 158th Batch (Delta, 199th verification, 2026-08-15)

**Evaluation Date**: 2026-08-15
**Evaluator**: Sisyphus (ULW Loop, autonomous run)
**Basis**: 198th audit matrix on `9b47f76`, re-executed fresh this run (see
`131-audit-report-2026-08-15-199th.md`).
**Convention**: GitHub issue creation is denied for this integration token (F002 —
`GraphQL: Resource not accessible by integration (createIssue)`, **187th
consecutive denial**, freshly probed this run). Per the 197-run docs-only
convention, findings are recorded in this ledger. Each entry carries the mandated
category + priority labels (contract §4).

## This run: 0 open PRs / 0 open issues → Phase 1 audit only — composite **70.4 (±0.0, 3rd flat)**, no new findings, no source delta; F037 100th obs, F038 38d, F002 187th, F005 99/47th, F018 26d, F008 RESOLVED maintained (41L + 11 modules), F029 NOT re-observed, 1121 JS + 27 py green, coverage 95.57/93.07, pull CI window 10/4/1 (46th-window `Endpoint is unavailable` re-confirmed infra-class from run log), pytest 27/27 holds

## Open findings (held)

| ID   | Category    | Priority | Title                                                                                                                          | State            |
| ---- | ----------- | -------- | ------------------------------------------------------------------------------------------------------------------------------ | ---------------- |
| F037 | security    | P0       | Workflow security: 12 violations (2 CRITICAL DUPLICATE_API_KEY + 10 HIGH)                                                      | HELD (100th obs) |
| F038 | ci          | P1       | Orchestrator workflow fails 10/10 (checkout exit 128, 38 days)                                                                 | HELD (38 days)   |
| F002 | chore       | P1       | GitHub issue creation denied for integration token (187th denial)                                                              | HELD             |
| F044 | security    | P2       | Over-scoped secrets: 10 unique names / 59 refs across workflows + template                                                     | HELD             |
| F005 | chore       | P2       | 99 docs/issues ledger files fail `prettier --check` (47th flat, held)                                                          | HELD/FLAT        |
...
## Run delta summary (199th vs 198th)

- **Composite**: **70.4 (±0.0)** — third consecutive flat run; no source-level
  delta since the 198th run's F008 split.
- **F037**: 99th → **100th observation** (12 violations, unchanged).
- **F002**: 186th → **187th consecutive denial** (issue creation still blocked).
- **F005**: 99 files, 46th → **47th consecutive flat** (all docs/issues; source
  re-checked 100% prettier-clean including the styles/ modules).
- **F063**: 47th window = this run (in progress). 46th-window failure
  (31906137151, 20:14Z) re-confirmed infra-class this run from the live log:
  `Upstream request failed: Endpoint is unavailable`, exit 1, no partial work.
...
## F063 detail (WATCH — no new data point this window)

This run (31911545953, 22:12Z) IS the 47th window and is in progress. No new
failure observation within this window at audit time. The last-15 window holds
**10 success + 4 failure + 1 in-progress**: the 4 failures (43rd 11:12Z, 44th
13:24Z, 45th 15:12Z, 46th 20:14Z) are all documented infra-class (two `Endpoint
is unavailable`, two opencode timeout exit 124). The 46th-window log was
re-fetched and re-verified this run — `Error: Upstream request failed: Endpoint
is unavailable` at 20:15:55Z, exit 1, no partial work, no PR. Zero code
regressions in any window failure; F063 stays WATCH with no repo-side action
warranted.
