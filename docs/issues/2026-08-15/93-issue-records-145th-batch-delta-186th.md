# Issue Records — 145th Batch (Delta, 186th verification, 2026-08-15)

**Evaluation Date**: 2026-08-15
**Evaluator**: Sisyphus (ULW Loop, autonomous run)
**Basis**: 185th audit matrix on `c7af197`, re-executed fresh this run (see
`92-audit-report-2026-08-15-186th.md`).
**Convention**: GitHub issue creation is denied for this integration token (F002 —
`GraphQL: Resource not accessible by integration (createIssue)`, **174th
consecutive denial**, freshly probed this run). Per the 185-run docs-only
convention, findings are recorded in this ledger. Each entry carries the mandated
category + priority labels (contract §4).

## This run: PR #726 merged (PR HANDLER MODE, prettier-clean at gate), orchestrator 10/10 HELD (28 days), pull CI best held 14/0/0 (35th window), F005 flat 99 (34th), F004 stable 59/10, F018 held at 26 days

## Open findings (held)

| ID   | Category    | Priority | Title                                                                         | State           |
| ---- | ----------- | -------- | ----------------------------------------------------------------------------- | --------------- |
| F037 | security    | P0       | Workflow security: 12 violations (2 CRITICAL DUPLICATE_API_KEY + 10 HIGH)     | HELD (87th obs) |
| F038 | ci          | P1       | Orchestrator workflow fails 10/10 (secrets.GH_TOKEN vs GITHUB_TOKEN, 28 days) | HELD (28 days)  |
| F002 | chore       | P1       | GitHub issue creation denied for integration token (174th denial)             | HELD            |
| F044 | security    | P2       | Over-scoped secrets: 10 unique names / 59 refs across workflows + template    | HELD            |
| F005 | chore       | P2       | 99 docs/issues ledger files fail `prettier --check` (34th flat, held)         | HELD/FLAT       |
...
| F063 | ci          | P1       | pull CI hourly timeout cluster (08-12) — SELF-RECOVERED, 35th clean window    | HEALTHY         |
