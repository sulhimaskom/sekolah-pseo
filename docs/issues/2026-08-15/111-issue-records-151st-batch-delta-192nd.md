# Issue Records — 151st Batch (Delta, 192nd verification, 2026-08-15)

**Evaluation Date**: 2026-08-15
**Evaluator**: Sisyphus (ULW Loop, autonomous run)
**Basis**: 191st audit matrix on `095ba4e`, re-executed fresh this run (see
`110-audit-report-2026-08-15-192nd.md`), plus the F065 source fix (Phase 2).
**Convention**: GitHub issue creation is denied for this integration token (F002 —
`GraphQL: Resource not accessible by integration (createIssue)`, **180th
consecutive denial**, freshly probed this run). Per the 191-run docs-only
convention, findings are recorded in this ledger. Each entry carries the mandated
category + priority labels (contract §4).

## This run: PR #732 merged (PR HANDLER MODE, prettier-clean at gate), **F065 homepage undefined-slug FIXED** (Phase 2 execution), orchestrator 10/10 HELD (33 days), pull CI best held 14/0/0 (41st window), F005 flat 99 (40th), F004 stable 59/10, F018 held at 26 days, composite 69.55 (+0.25)

## Open findings (held)

| ID   | Category    | Priority | Title                                                                         | State           |
| ---- | ----------- | -------- | ----------------------------------------------------------------------------- | --------------- |
| F037 | security    | P0       | Workflow security: 12 violations (2 CRITICAL DUPLICATE_API_KEY + 10 HIGH)     | HELD (93rd obs) |
| F038 | ci          | P1       | Orchestrator workflow fails 10/10 (secrets.GH_TOKEN vs GITHUB_TOKEN, 33 days) | HELD (33 days)  |
| F002 | chore       | P1       | GitHub issue creation denied for integration token (180th denial)             | HELD            |
| F044 | security    | P2       | Over-scoped secrets: 10 unique names / 59 refs across workflows + template    | HELD            |
| F005 | chore       | P2       | 99 docs/issues ledger files fail `prettier --check` (40th flat, held)         | HELD/FLAT       |
...
| F063 | ci          | P1       | pull CI hourly timeout cluster (08-12) — SELF-RECOVERED, 41st clean window    | HEALTHY         |

## RESOLVED this run

| ID   | Category | Priority | Title                                            | State    |
| ---- | -------- | -------- | ------------------------------------------------ | -------- |
| F065 | bug      | P2       | Homepage undefined-slug latent bug (homepage.js) | RESOLVED |

F065 resolution: `generateHomepageHtml()` now normalizes non-array input via
`Array.isArray(schools) ? schools : []` before the `schools.length` access,
matching the guard already present in `aggregateProvinceAndFilters()` (same
file). Regression test added (`scripts/homepage.test.js`: "renders empty
homepage for undefined input"). Full matrix re-verified green after the fix
(lint 0/0, build PASS, test:js 1101/0/4, pytest 13/13, prettier-clean).
