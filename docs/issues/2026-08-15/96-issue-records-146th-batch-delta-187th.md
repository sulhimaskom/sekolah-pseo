# Issue Records — 146th Batch (Delta, 187th verification, 2026-08-15)

**Evaluation Date**: 2026-08-15
**Evaluator**: Sisyphus (ULW Loop, autonomous run)
**Basis**: 186th audit matrix on `6b23b3e`, re-executed fresh this run (see
`95-audit-report-2026-08-15-187th.md`).
**Convention**: GitHub issue creation is denied for this integration token (F002 —
`GraphQL: Resource not accessible by integration (createIssue)`, **175th
consecutive denial**, freshly probed this run). Per the 186-run docs-only
convention, findings are recorded in this ledger. Each entry carries the mandated
category + priority labels (contract §4).

## This run: PR #727 merged (PR HANDLER MODE, prettier-clean at gate), orchestrator 10/10 HELD (28 days), pull CI best held 14/0/0 (36th window), F005 flat 99 (35th), F004 stable 59/10, F018 held at 26 days

## Open findings (held)

| ID   | Category    | Priority | Title                                                                         | State           |
| ---- | ----------- | -------- | ----------------------------------------------------------------------------- | --------------- |
| F037 | security    | P0       | Workflow security: 12 violations (2 CRITICAL DUPLICATE_API_KEY + 10 HIGH)     | HELD (88th obs) |
| F038 | ci          | P1       | Orchestrator workflow fails 10/10 (secrets.GH_TOKEN vs GITHUB_TOKEN, 28 days) | HELD (28 days)  |
| F002 | chore       | P1       | GitHub issue creation denied for integration token (175th denial)             | HELD            |
| F044 | security    | P2       | Over-scoped secrets: 10 unique names / 59 refs across workflows + template    | HELD            |
| F005 | chore       | P2       | 99 docs/issues ledger files fail `prettier --check` (35th flat, held)         | HELD/FLAT       |
...
| F063 | ci          | P1       | pull CI hourly timeout cluster (08-12) — SELF-RECOVERED, 36th clean window    | HEALTHY         |
