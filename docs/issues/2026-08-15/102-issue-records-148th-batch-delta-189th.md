# Issue Records — 148th Batch (Delta, 189th verification, 2026-08-15)

**Evaluation Date**: 2026-08-15
**Evaluator**: Sisyphus (ULW Loop, autonomous run)
**Basis**: 188th audit matrix on `be18878`, re-executed fresh this run (see
`101-audit-report-2026-08-15-189th.md`).
**Convention**: GitHub issue creation is denied for this integration token (F002 —
`GraphQL: Resource not accessible by integration (createIssue)`, **177th
consecutive denial**, freshly probed this run). Per the 188-run docs-only
convention, findings are recorded in this ledger. Each entry carries the mandated
category + priority labels (contract §4).

## This run: PR #729 merged (PR HANDLER MODE, prettier-clean at gate), orchestrator 10/10 HELD (30 days), pull CI best held 14/0/0 (38th window), F005 flat 99 (37th), F004 stable 59/10, F018 held at 26 days

## Open findings (held)

| ID   | Category    | Priority | Title                                                                         | State           |
| ---- | ----------- | -------- | ----------------------------------------------------------------------------- | --------------- |
| F037 | security    | P0       | Workflow security: 12 violations (2 CRITICAL DUPLICATE_API_KEY + 10 HIGH)     | HELD (90th obs) |
| F038 | ci          | P1       | Orchestrator workflow fails 10/10 (secrets.GH_TOKEN vs GITHUB_TOKEN, 30 days) | HELD (30 days)  |
| F002 | chore       | P1       | GitHub issue creation denied for integration token (177th denial)             | HELD            |
| F044 | security    | P2       | Over-scoped secrets: 10 unique names / 59 refs across workflows + template    | HELD            |
| F005 | chore       | P2       | 99 docs/issues ledger files fail `prettier --check` (37th flat, held)         | HELD/FLAT       |
...
| F019 | refactor    | P3       | run_tests.py duplicate imports (lines 12–19 vs 20–25)                         | HELD            |
...
| F063 | ci          | P1       | pull CI hourly timeout cluster (08-12) — SELF-RECOVERED, 38th clean window    | HEALTHY         |
