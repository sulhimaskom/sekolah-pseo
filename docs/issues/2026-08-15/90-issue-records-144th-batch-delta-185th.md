# Issue Records — 144th Batch (Delta, 185th verification, 2026-08-15)

**Evaluation Date**: 2026-08-15
**Evaluator**: Sisyphus (ULW Loop, autonomous run)
**Basis**: 184th audit matrix on `b891f99`, re-executed fresh this run (see
`89-audit-report-2026-08-15-185th.md`).
**Convention**: GitHub issue creation is denied for this integration token (F002 —
`GraphQL: Resource not accessible by integration (createIssue)`, **173rd
consecutive denial**, freshly probed this run). Per the 184-run docs-only
convention, findings are recorded in this ledger. Each entry carries the mandated
category + priority labels (contract §4).

## This run: PR #725 merged (PR HANDLER MODE, prettier-clean at gate), orchestrator 10/10 HELD (28 days), pull CI best held 14/0/0 (34th window), F005 flat 99 (33rd), F004 stable 59/10, F018 drifted to 26 days

## Open findings (held)

| ID   | Category    | Priority | Title                                                                         | State           |
| ---- | ----------- | -------- | ----------------------------------------------------------------------------- | --------------- |
| F037 | security    | P0       | Workflow security: 12 violations (2 CRITICAL DUPLICATE_API_KEY + 10 HIGH)     | HELD (86th obs) |
| F038 | ci          | P1       | Orchestrator workflow fails 10/10 (secrets.GH_TOKEN vs GITHUB_TOKEN, 28 days) | HELD (28 days)  |
| F002 | chore       | P1       | GitHub issue creation denied for integration token (173rd denial)             | HELD            |
| F044 | security    | P2       | Over-scoped secrets: 10 unique names / 59 refs across workflows + template    | HELD            |
| F005 | chore       | P2       | 99 docs/issues ledger files fail `prettier --check` (33rd flat, held)         | HELD/FLAT       |
...
| F018 | enhancement | P1       | Data freshness watchdog STALE (26 days > 7 threshold; drifted 25 → 26)        | HELD            |
...
| F063 | ci          | P1       | pull CI hourly timeout cluster (08-12) — SELF-RECOVERED, 34th clean window    | HEALTHY         |
