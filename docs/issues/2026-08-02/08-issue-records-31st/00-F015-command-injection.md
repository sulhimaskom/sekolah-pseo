# F015 — OS Command Injection in fetch-data.js (EXPLOITABLE, PoC-confirmed 9th consecutive run)

**Evaluation Date**: 2026-08-02 (31st run)
**Category**: security
**Priority**: P1
**Status**: OPEN — **EXPLOITABLE (live, 9th consecutive)**
...
## PoC (re-verified this run, 31st — validation harness, no execution)
...
- `https://github.com/foo/%60id%60.git` → backtick substitution executes `id`
- `https://github.com/foo/bar && rm -rf /tmp/x.git` → `&&` chain (space URL-encoded to `%20`,
  still passed through) — additional payload class re-verified this run

Verified via direct replication of the exact `validateRepoUrl` reconstruction logic: all four
payloads `PASS` through and reach the `git clone` string interpolation. **No payload executed
during verification** (harness only calls the validator, never `execSync`).
...
- 30th run: PoC-confirmed 8th
- **31st run: PoC-confirmed 9th** (fresh harness this run; +`&&` payload class)
