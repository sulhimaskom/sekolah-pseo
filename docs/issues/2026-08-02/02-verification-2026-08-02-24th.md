# Phase 1 — Comprehensive Scoring Report (ULW Loop) — 2026-08-02 (24th verification)

**Evaluation Date**: 2026-08-02
**Evaluator**: Sisyphus (ULW Loop, autonomous run)
**Repository**: sulhimaskom/sekolah-pseo
**Default Branch**: main (@ 3361b8e)
**Trigger**: ULW loop invocation (Phase 0 → no open PRs, no open issues → Phase 1)
...
| Domain                                | Score        | Grade |
| ------------------------------------- | ------------ | ----- |
| **A. Code Quality**                   | **81.8/100** | B     |
| **B. System Quality**                 | **76.3/100** | B     |
| **C. Experience Quality**             | **85.2/100** | B     |
| **D. Delivery & Evolution Readiness** | **70.1/100** | C+    |
| **COMPOSITE**                         | **78.4/100** | B     |

Delta vs 2026-08-02 23rd run (78.7): **−0.3** — driven by **F014 parallel
test-file race OBSERVED at highest frequency yet (5/13 runs)** and F015 RCE
PoC re-confirmed live (4th consecutive confirmation). Prettier file count up
17→18 (F005). No application-code regression.

## Global Penalties

| Rule                   | Penalty | Justification                                                                            |
| ---------------------- | ------- | ---------------------------------------------------------------------------------------- |
| Build failure          | —       | ✅ PASS — `npm run build` → exit 0, 2 pages, 0 failed, 37ms, budgets met                 |
| Test failure           | −15     | ⚠️ F014 parallel test-file race **OBSERVED 5/13 runs** (highest frequency in 6 sessions) |
| Critical vulnerability | −20     | ❌ F015 OS command injection PoC-confirmed (4th live confirmation)                       |
...
| `npm run build`                           | ✅ exit 0, 2 pages, 0 failed, 37ms, 54.05 pages/sec, 57.34MB RSS, budgets met   |
| `npm run lint` (eslint)                   | ✅ clean — 0 errors, 0 warnings                                                 |
| `npm run test:js` (×13)                   | ⚠️ nominal 1026 pass / 0 fail / 4 skip; **5 failures observed in 13 runs**      |
...
| `npm run format:check`                    | ❌ **18 files unformatted** (up from 17)                                        |
| `git tag`                                 | ❌ 0 tags (finding 011)                                                         |
| F015 live PoC (`validateRepoUrl`)         | ❌ **`bar;id.git` and `bar$(id).git` both ACCEPTED** → reach `execSync`         |

---

## A. CODE QUALITY (Weighted: 81.8/100)
...
| Testability                  | 15      | 80    | 12.00     | Coverage 95.32/92.28 met; pytest not wired (009); F014 freq rising  |
...
| Determinism & Predictability | 5       | 68    | 3.40      | **F014 OBSERVED (5/13)** — highest in 6 sessions; F001; ETL churn   |
| **TOTAL**                    | **100** |       | **81.80** |                                                                     |
...
- **Observations**: Full suite nominally green (1026 JS pass; 27 Python pass). Two latent
  ...
  reach the `git clone` command string (verified live this run). (b) **F001** —
  `fetch-data.js:338` `fetchFromGitHub(sourceRepo)` not awaited in sync `main()` (line 319).
...
- **Observations**: Coverage 95.32% stmt / 92.28% branch — above 80/75 thresholds. 97
  ...
  **F014 race observed 5/13 runs this session — exact failure captured**.
- **Evidence**: `npm run test:js:coverage` output; F014 failure: `build-orchestrator.test.js:153`
  `prepareBuildEnvironment` suite (ERR_ASSERTION, subtest failed).
- **Impact**: Medium — Python tests are thin; F014 makes CI intermittently red.
- **Deductions**: −5 for F014 test-file race surfacing at rising frequency.

### A7. Maintainability (78/100)

- **Observations**: styles.js (1275L) and workflow files (2045L total) concentrate risk;
  18 unformatted docs files; silent catches in enrichment.js:318, PageBuilder.js:112.
...
### A10. Determinism (68/100) ⚠️

- **Observations**: Content-hash incremental builds, no global state — but **F014 test
  race OBSERVED (5/13 runs) this session — highest observed frequency in 6 sessions**
  (rising: 0/3 → 4/28 → 3/31 → 5/14 → 0/6 → 3/11 → 5/13), F001 floating promise,
  ETL `updated_at` churn + sitemap `lastmod`=today.
- **Evidence**: 13 full-suite runs this session (5 fail / 8 clean); findings 001, 014.
- **Impact**: Low-moderate — intermittent CI red without code change; frequency trending up.

---

## B. SYSTEM QUALITY (RUNTIME) (Weighted: 76.3/100)

| Criterion                    | Weight  | Score | Weighted  | Rationale                                                                  |
| ---------------------------- | ------- | ----- | --------- | -------------------------------------------------------------------------- |
| Stability                    | 20      | 74    | 14.80     | **F014 observed 5/13 (highest)**; CI no gate before 12 AI flows (007)      |
| Performance Efficiency       | 15      | 90    | 13.50     | 37ms build, 54.05 pages/sec, budgets met, 57.34MB RSS                      |
...
### B1. Stability (74/100) ⚠️

- **Observations**: Build passes consistently; F014 **OBSERVED 5/13 runs** (highest
  frequency in 6 sessions; trend rising). CI still runs 12 sequential AI flows with no
  build/lint/test gate.
- **Evidence**: 13 full-suite runs this session; failure captured at
  `build-orchestrator.test.js:153` (`prepareBuildEnvironment`, `dist/` race); finding 007.
- **Deductions**: −16 F014 observed; −10 CI not gating (on-push.yml).

### B2. Performance (90/100)

- **Observations**: 37ms build, 54.05 pages/sec, 57.34MB peak RSS, all budgets met.
...
- **Observations**: 0 npm vulns, path-traversal/XSS protections present. BUT:
  - **F015 OS command injection** (PoC re-confirmed live this run — 4th consecutive) —
  ...
  - **57 `secrets.*` references** across workflows, 10 distinct secret names (finding 004).
- **Evidence**: Live PoC (`validateRepoUrl` accepts `;id`/`$(id)` — verified this run);
  `node scripts/check-workflow-security.js` → 12 violations.
...
| Documentation Accuracy   | 70    | 18 unformatted docs (005, up from 17); `gitignore-check` ghost (F016); `addNumbers` ghost (F017) |
| Debuggability            | 80    | Structured logging, named errors, build perf metrics                                             |
| Build/Test Feedback Loop | 95    | Build 37ms, JS tests <5s, coverage gate enforced                                                 |

**DX Total**: (88+90+70+80+95)/5 = 84.6
**Experience Quality**: (85.75 + 84.6) / 2 = **85.2**
...
| Technical Debt Exposure        | 15      | 66    | 9.90      | styles.js 1275L; 18 unformatted docs (005); lint-staged engine mismatch (012)        |
...
- **Observations**: 6 workflows, 2045 lines total. on-push.yml (533L) runs 12 sequential
  ...
  `workflows: write` — **root cause of 24 blocked audit runs** (fresh 403 this run).
...
- **Observations**: Centralized config with env support, but SITE_URL defaults to
  ...
  `config.js:50-54`); no startup env validation; PERF_* env vars undocumented.
...
### D5. Technical Debt (66/100)

- **Observations**: styles.js 1275L; **18 unformatted docs files (up from 17)**; lint-staged
...
| 002 | Missing `issues: write` + `workflows: write` (loop runner) | ci       | P1  | ✅ valid (24th consecutive)   | `gh issue create` → 403 `createIssue`                                    |
...
| 005 | Prettier violations in docs                                | docs     | P3  | ✅ valid                      | **18 files unformatted** (up from 17)                                    |
...
| 014 | Parallel test-file race (`dist/` + `external-data/`)       | test     | P1  | ✅ valid — **OBSERVED 5/13**  | `build-orchestrator.test.js:153` ERR_ASSERTION captured; 5 fails/13 runs |
...
- `gh issue create --title "PERMISSION-TEST <ts>"` → `GraphQL: Resource not accessible by
integration (createIssue)`
- Runner token grants `contents: write`, `pull-requests: write` — **no `issues: write`**
- This is the **24th consecutive blocked run** (finding 002).
...
- **Composite Score**: 78.4/100 (−0.3 vs 23rd run, F014 observed 5/13 — highest frequency
  in 6 sessions; F015 PoC re-confirmed 4th time)
