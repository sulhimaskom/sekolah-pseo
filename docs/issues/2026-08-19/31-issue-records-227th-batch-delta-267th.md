# Issue Records — 227th Batch Delta (267th run, 2026-08-19)

**Ledger**: 227th batch delta. **Trigger**: Phase 0 → 1 open PR (#828) → PR HANDLER MODE (merged) → re-probe 0 open PRs / 0 open issues → Phase 0.3 EMPTY → Phase 1 audit (267th, read-only flat verification).

**F002 status**: `gh api graphql createIssue` (repository node id `R_kgDOQKx7JA`) → GraphQL "Resource not accessible by integration" — **403 CONCLUSIVE (69th)** — token lacks `issues: write`. GitHub-native issue creation remains blocked. All findings recorded here in the ledger, the established output channel.

## Finding status table (267th basis — active tracked findings)

| Finding | Category | Priority | Status | Observation |
| ------- | -------- | -------- | ------ | ----------- |
| F002    | ci       | P1       | HELD   | Token lacks `issues: write` — **403 CONCLUSIVE (69th)** (re-probed direct this run, correct node id) |
| F004    | security | P2       | HELD   | **57 refs / 10 unique names** (workflows `*.yml` only) — byte-identical to 266th basis (0 workflow commits since `e00195f`), **zero growth** |
| F005    | docs     | P3       | HELD   | **196 files** (115th obs): 195 ledger + SECURITY_AUDIT_NOTE.md — **zero source files** (F251 verified holding, 18th clean obs); ledger-only drift by convention |
...
| F037    | security | P1       | HELD   | 12 workflow violations (2 CRITICAL + 10 HIGH), **168th obs**, push-blocked F050 (44th documented pass) |
| F038    | ci       | P1       | HELD   | orchestrator workflow **6/6 most recent FAILED** (~66+ days, latest 2026-08-19T00:50:16Z) |
| F050    | ci       | P1       | HELD   | token lacks `workflows: write` — workflow-file changes blocked (44th documented pass) |
| F063    | ci       | P1       | HELD (improving) | on-pull **29/29 completed successes** visible, 0 failures + 1 in-progress (14:25Z — this run's PR-merge-triggered); **streak ≥24 maintained** |
| F064    | ci       | P3       | HELD   | `.nvmrc` 22 vs runtime node v20.20.2 vs CI node 20 vs devcontainer node 20; lint-staged EBADENGINE (wants node ≥22.22.1); **pytest env-parity UNAVAILABLE this run (13th obs — continued unavailable)** |
...
| F228    | docs     | P2       | RESOLVED maintained | README.md:305-319 / SECURITY.md:46-50 / deployment.md accurate at HEAD **`c85f51b`** (**29th clean obs**) |
| F229    | security | P1       | HELD   | commit `0056ad8` still UNREACHABLE (**32nd obs**); unhardened `curl -fsSL https://opencode.ai/install \| bash` (no pipefail/retry) in **on-pull.yml:63 AND on-push.yml:63** |
| F230    | docs     | P3       | RESOLVED maintained | `prettier --check docs/blueprint.md docs/task.md` clean (**31st clean obs**) |
| F231    | bug      | P2       | RESOLVED maintained | monitorBuild report-after-stop verified at source (build-performance.js:344) — **21st clean obs** |
| F232    | bug      | P1       | RESOLVED maintained | manifest hash includes lat/lon (manifest.js:130-140) — **21st clean obs** |
...
| F234    | security | P1       | RESOLVED maintained | EXTERNAL_DATA_DIR injection closed (fetch-data.js:183-198) — **21st clean obs** |
...
| F239    | docs     | P2       | RESOLVED maintained | api.md:3888 Comparison Module + api.md:6627 Test Helpers Module present (**17th obs**) |
...
| F251    | chore    | P3       | RESOLVED maintained | `scripts/data-schema.test.js` prettier-clean (**18th clean obs** — zero source drift invariant) |
...
## Suggested resolutions for open HELD findings (unchanged from 226th batch, held)
...
- `docs/issues/2026-08-19/30-audit-report-2026-08-19-267th.md`
- `docs/issues/2026-08-19/31-issue-records-227th-batch-delta-267th.md` (this file)
- `docs/issues/2026-08-19/32-phase2-3-decision-267th.md`