# Issue Records — 221st Batch Delta (261st run, 2026-08-19)

**Ledger**: 221st batch delta. **Trigger**: Phase 0 → 1 open PR (#822) → PR HANDLER MODE (merged) → re-probe 0 open PRs / 0 open issues → Phase 0.3 EMPTY → Phase 1 audit (261st, read-only flat verification).

**F002 status**: `gh api graphql createIssue` → GraphQL "Resource not accessible by integration" — **403 CONCLUSIVE (63rd)** — token lacks `issues: write`. GitHub-native issue creation remains blocked. All findings recorded here in the ledger, the established output channel.

## Finding status table (261st basis — active tracked findings)

| Finding | Category | Priority | Status | Observation |
| ------- | -------- | -------- | ------ | ----------- |
| F002    | ci       | P1       | HELD   | Token lacks `issues: write` — **403 CONCLUSIVE (63rd)** (re-probed direct this run with real repo node id `R_kgDOQKx7JA`) |
| F004    | security | P2       | HELD   | **57 refs / 10 unique names** (workflows `*.yml` only) — byte-identical to 260th basis, **zero growth** |
| F005    | docs     | P3       | HELD   | **178 files** (109th obs): 177 ledger + SECURITY_AUDIT_NOTE.md — **zero source files** (F251 verified holding, 12th clean obs); ledger-only drift by convention |
...
| F037    | security | P1       | HELD   | 12 workflow violations (2 CRITICAL + 10 HIGH), **162nd obs**, push-blocked F050 (38th documented pass) |
...
| F045    | bug      | P1       | HELD (root cause FIXED) | stale-page accumulation — F232 fix verified holding (15th obs); residual surface (school deletion/move) held |
| F046    | bug      | P2       | HELD   | whole-build abort on single bad row |
| F050    | ci       | P1       | HELD   | GitHub App token lacks `workflows` permission — blocks workflow-file delivery (38th push-block pass) |
| F063    | ci       | P1       | HELD (IMPROVING) | on-pull: **≥24 consecutive completed successes maintained** (5 visible + 1 in-progress 08:27:29Z — this run's PR-merge-triggered); no NEW failure |
| F064    | config   | P3       | HELD   | `.nvmrc` 22 vs runtime/CI/devcontainer node 20 (Node 20 EOL); lint-staged EBADENGINE (node ≥22.22.1) surfaced at install; **pytest unavailable 7th consecutive run (255th → 256th → 257th → 258th → 259th → 260th → 261st, env-parity)** |
...
| F228    | docs     | P3       | **RESOLVED maintained** | README.md:294-304, SECURITY.md:46-50, deployment.md:62-66 accurate at HEAD — **23rd clean obs** |
| F229    | ci       | P1       | HELD   | TASK-102 fix commit `0056ad8` UNREACHABLE (**26th obs**); unhardened `curl\|bash` installer in **on-pull.yml:63 AND on-push.yml:63** |
| F230    | docs     | P3       | **RESOLVED maintained** | blueprint.md/task.md prettier-clean at HEAD (**25th clean obs**) |
| F231    | bug      | P2       | **FIXED (verified 15th obs)** | monitorBuild report generated after stop() — returned metrics real (build-performance.js:344) |
| F232    | bug      | P1       | **FIXED (verified 15th obs)** | computeSchoolHash includes lat/lon — incremental rebuild on coordinate-only change (manifest.js:130-140) |
...
| F234    | security | P1       | **FIXED (verified 15th obs)** | validateExternalDataDir rejects shell metacharacters + path traversal before git clone interpolation (fetch-data.js:185-196) |
...
| F239    | docs     | P2       | **FIXED (verified 11th maintained obs)** | **all tree-listed modules have api.md sections** — Comparison + Test Helpers sections present at HEAD |
...
| F251    | test     | P3       | **FIXED (verified 12th clean obs)** | `scripts/data-schema.test.js` prettier-clean at HEAD — zero-source-drift invariant held |
...
## NEW findings this run (261st)

None. Read-only flat verification run (with PR handler activity) — every probe held at 260th levels. Only deltas: (1) pytest availability remains unavailable 7th consecutive run (255th → 256th → 257th → 258th → 259th → 260th → 261st, env-parity — F064-adjacent, noted in audit report §Environment parity note), (2) ledger prettier count 176 → 178 (3 new ledger files from the 260th run merged via PR #822; zero source), (3) F063 completed-success streak maintained ≥24, (4) F038 failure window held (~65+ days), (5) F229 unreachable 25th → 26th obs (unhardened `curl|bash` installer confirmed in **both** on-pull.yml:63 and on-push.yml:63), (6) F037 161st → 162nd obs, (7) F002 62nd → 63rd obs (re-probed with real repo node id `R_kgDOQKx7JA` this run).

## Deferred/blocked (recorded, not this run)

| Finding | Category | Priority | Rationale |
| ------- | -------- | -------- | --------- |
| F229/F063/F037/F038/F227/F252/F253 | ci/security | P1/P2 | workflow hardening requires `workflows: write` — token lacks it (F050, 38th documented pass) |
