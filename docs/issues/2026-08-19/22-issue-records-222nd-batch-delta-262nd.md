# Issue Records — 222nd Batch Delta (262nd run, 2026-08-19)

**Ledger**: 222nd batch delta. **Trigger**: Phase 0 → 1 open PR (#823) → PR HANDLER MODE (merged) → re-probe 0 open PRs / 0 open issues → Phase 0.3 EMPTY → Phase 1 audit (262nd, read-only flat verification).

**F002 status**: `gh api graphql createIssue` → GraphQL "Resource not accessible by integration" — **403 CONCLUSIVE (64th)** — token lacks `issues: write`. GitHub-native issue creation remains blocked. All findings recorded here in the ledger, the established output channel.

## Finding status table (262nd basis — active tracked findings)

| Finding | Category | Priority | Status | Observation |
| ------- | -------- | -------- | ------ | ----------- |
| F002    | ci       | P1       | HELD   | Token lacks `issues: write` — **403 CONCLUSIVE (64th)** (re-probed direct this run) |
| F004    | security | P2       | HELD   | **57 refs / 10 unique names** (workflows `*.yml` only) — byte-identical to 261st basis, **zero growth** |
| F005    | docs     | P3       | HELD   | **181 files** (110th obs): 180 ledger + SECURITY_AUDIT_NOTE.md — **zero source files** (F251 verified holding, 13th clean obs); ledger-only drift by convention |
...
| F037    | security | P1       | HELD   | 12 workflow violations (2 CRITICAL + 10 HIGH), **163rd obs**, push-blocked F050 (39th documented pass) |
...
| F045    | bug      | P1       | HELD (root cause FIXED) | stale-page accumulation — F232 fix verified holding (16th obs); residual surface (school deletion/move) held |
| F046    | bug      | P2       | HELD   | whole-build abort on single bad row |
| F050    | ci       | P1       | HELD   | GitHub App token lacks `workflows` permission — blocks workflow-file delivery (39th push-block pass) |
| F063    | ci       | P1       | HELD (IMPROVING) | on-pull: **≥24 consecutive completed successes maintained** (5 visible + 1 in-progress 09:25:36Z — this run's PR-merge-triggered); no NEW failure |
| F064    | config   | P3       | HELD   | `.nvmrc` 22 vs runtime/CI/devcontainer node 20 (Node 20 EOL); lint-staged EBADENGINE (node ≥22.22.1) surfaced at install; **pytest unavailable 8th consecutive run (255th → 256th → 257th → 258th → 259th → 260th → 261st → 262nd, env-parity)** |
...
| F228    | docs     | P3       | **RESOLVED maintained** | README.md:294-304, SECURITY.md:46-50, deployment.md:62-66 accurate at HEAD — **24th clean obs** |
| F229    | ci       | P1       | HELD   | TASK-102 fix commit `0056ad8` UNREACHABLE (**27th obs**); unhardened `curl\|bash` installer in **on-pull.yml:63 AND on-push.yml:63** |
| F230    | docs     | P3       | **RESOLVED maintained** | blueprint.md/task.md prettier-clean at HEAD (**26th clean obs**) |
| F231    | bug      | P2       | **FIXED (verified 16th obs)** | monitorBuild report generated after stop() — returned metrics real (build-performance.js:344) |
| F232    | bug      | P1       | **FIXED (verified 16th obs)** | computeSchoolHash includes lat/lon — incremental rebuild on coordinate-only change (manifest.js:130-140) |
...
| F234    | security | P1       | **FIXED (verified 16th obs)** | validateExternalDataDir rejects shell metacharacters + path traversal before git clone interpolation (fetch-data.js:185-196) |
...
| F239    | docs     | P2       | **FIXED (verified 12th maintained obs)** | **all tree-listed modules have api.md sections** — Comparison + Test Helpers sections present at HEAD |
...
| F251    | test     | P3       | **FIXED (verified 13th clean obs)** | `scripts/data-schema.test.js` prettier-clean at HEAD — zero-source-drift invariant held |
...
## NEW findings this run (262nd)

None. Read-only flat verification run (with PR handler activity) — every probe held at 261st levels. Only deltas: (1) pytest availability remains unavailable 8th consecutive run (255th → 256th → 257th → 258th → 259th → 260th → 261st → 262nd, env-parity — F064-adjacent, noted in audit report §Environment parity note), (2) ledger prettier count 178 → 181 (3 new ledger files from the 261st run merged via PR #823; zero source), (3) F063 completed-success streak maintained ≥24, (4) F038 failure window held (~65+ days), (5) F229 unreachable 26th → 27th obs (unhardened `curl|bash` installer confirmed in **both** on-pull.yml:63 and on-push.yml:63), (6) F037 162nd → 163rd obs, (7) F002 63rd → 64th obs.

## Deferred/blocked (recorded, not this run)

| Finding | Category | Priority | Rationale |
| ------- | -------- | -------- | --------- |
| F229/F063/F037/F038/F227/F252/F253 | ci/security | P1/P2 | workflow hardening requires `workflows: write` — token lacks it (F050, 39th documented pass) |
