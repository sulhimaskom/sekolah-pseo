# Issue Records — 187th Batch (Delta, 228th verification, 2026-08-17)

**Evaluation Date**: 2026-08-17
**Evaluator**: Sisyphus (ULW Loop, autonomous run)
**Basis**: 227th audit matrix on `169c473`, re-executed fresh this run on `50a229b`
(see `218-audit-report-2026-08-17-228th.md`).
**Convention**: GitHub issue creation is denied for this integration token (F002 —
`GraphQL: Resource not accessible by integration (createIssue)`, **216th
consecutive denial**, freshly probed this run). Per the 217-run docs-only
convention, findings are recorded in this ledger. Each entry carries the
mandated category + priority labels (contract §4).

## This run: 0 open PRs / 0 open issues → Phase 1 audit only — composite **70.4 (±0.0, 32nd flat)**, no new findings, no source delta; F037 129th obs, F038 49d (15/15), F002 216th, F005 102/76th, F018 **28d held**, F008 RESOLVED maintained (41L + 11 modules), F029 NOT re-observed, 1121 JS + 13 py + 27 wrapper green, coverage 95.57/93.07, pull CI window **24/0/1 zero-failure maintained** (97th win in-prog at 08:37Z), F063 IMPROVING

## Open findings (held)

| ID   | Category    | Priority | Title                                                                                                                              | State            |
| ---- | ----------- | -------- | ---------------------------------------------------------------------------------------------------------------------------------- | ---------------- |
| F037 | security    | P0       | Workflow security: 12 violations (2 CRITICAL DUPLICATE_API_KEY + 10 HIGH)                                                          | HELD (129th obs) |
| F038 | ci          | P1       | Orchestrator workflow fails 15/15 (checkout exit 128, 49 days)                                                                     | HELD (49 days)   |
| F002 | chore       | P1       | GitHub issue creation denied for integration token (216th denial)                                                                  | HELD             |
| F044 | security    | P2       | Over-scoped secrets: 10 unique names / 59 refs across workflows + template                                                         | HELD             |
| F005 | chore       | P2       | 102 docs/issues ledger files fail `prettier --check` (76th flat, held)                                                             | HELD/FLAT        |
...
| F063 | ci          | P1       | pull CI hourly — **24/0/1, zero-failure window maintained**; 08:37Z in-progress = 97th win; F038 remains the structural CI failure | IMPROVING        |
...
history is docs-only through `50a229b`, so no new source surface exists):
...
in a clean, buildable, green state (see `218-audit-report-2026-08-17-228th.md`).
