# Issue Records — 124th Batch (Delta, 165th verification, 2026-08-13)

**Evaluation Date**: 2026-08-13
**Evaluator**: Sisyphus (ULW Loop, autonomous run)
**Basis**: 164th audit matrix on `e6f1d23`, re-executed fresh this run (see
`44-audit-report-2026-08-13-165th.md`).
**Convention**: GitHub issue creation is denied for this integration token (F002 —
`GraphQL: Resource not accessible by integration (createIssue)`, **153rd
consecutive denial**, freshly probed this run). Per the 164-run docs-only
convention, findings are recorded in this ledger. Each entry carries the mandated
category + priority labels (contract §4).

## This run: PR #705 merged (PR HANDLER MODE), orchestrator 10/10 HELD (15 days), pull CI best held 11/1/0 (14th window), F005 flat 96 (13th), F004 stable 59/10

## Open findings (held)

| ID   | Category    | Priority | Title                                                                         | State           |
| ---- | ----------- | -------- | ----------------------------------------------------------------------------- | --------------- |
| F037 | security    | P0       | Workflow security: 12 violations (2 CRITICAL DUPLICATE_API_KEY + 10 HIGH)     | HELD (66th obs) |
| F038 | ci          | P1       | Orchestrator workflow fails 10/10 (secrets.GH_TOKEN vs GITHUB_TOKEN, 15 days) | HELD (15 days)  |
| F002 | chore       | P1       | GitHub issue creation denied for integration token (153rd denial)             | HELD            |
| F044 | security    | P2       | Over-scoped secrets: 10 unique names / 59 refs across workflows + template    | HELD            |
| F005 | chore       | P2       | 96 docs/issues ledger files fail `prettier --check` (13th flat)               | HELD/FLAT       |
...
| F063 | ci          | P1       | pull CI hourly timeout cluster (08-12) — SELF-RECOVERED, 14th clean window    | HEALTHY         |
