# Issue Records — 91st Batch (Delta, 132nd verification, 2026-08-11)

**Evaluation Date**: 2026-08-11
**Evaluator**: Sisyphus (ULW Loop, autonomous run)
**Basis**: 131st audit matrix on `7333e5b` (see `15-audit-report-2026-08-11-131st.md`).
**Convention**: GitHub issue creation is denied for this integration token (F002 —
`GraphQL: Resource not accessible by integration (createIssue)`, 120th consecutive
denial). Per the 129-run docs-only convention, findings are recorded in this ledger.

## Stability confirmation (no action — held findings)

All labeled findings from the 90th-batch records remain HELD with byte-identical
...
| F037 | security/P0       | HELD   | opencode.yml/parallel.yml duplicate `GEMINI_API_KEY` — CRITICAL (33rd)        |
...
| F002 | chore/P1          | HELD   | `gh issue create` denied (120th)                                              |
...
### R-132-1: Build timing variance within budget (chore, P3)

- **Finding**: full build 27ms / 74.07 pages/sec this run vs 46ms / 43.48 pages/sec at
  the 131st — run-to-run noise on a 2-school sample; budgets still PASS.
- **Files affected**: none (observation only).
- **Labels**: `chore`, `P3`.

### R-132-2: SITE_URL placeholder warning re-observed (chore, P2)
...
- **Evidence**: `node scripts/check-freshness.js` → "SITE_URL is set to default
  placeholder \"https://example.com\"" (re-observed 4th run). Config reference:
...
### R-132-3: orchestrator failure sample consistent (ci, P1)

- **Finding**: last 5 scheduled `oc - orchestrator` runs all `failure` — consistent
  with the 131st's 5/5. Root cause unchanged: checkout cannot authenticate
...
### R-132-4: F068 pytest re-observed, resolved in-loop (test, P2)
...
| ----------------- | --------------------- | ------------------------------------------ | -------------------------------------------------------------------------------------------------------- |
| 2026-08-11 12:47Z | Phase 0 probe         | `gh pr list` / `gh issue list`             | 0 open PRs / 0 issues → PHASE 1 (AUDIT MODE)                                                             |
| 2026-08-11 12:47Z | skills survey         | `.opencode/skill/` (7)                     | identified; audit executed directly (convention)                                                         |
| 2026-08-11 12:50Z | `npm ci`              | node_modules (clean)                       | 131 packages, 0 vulnerabilities, EBADENGINE (lint-staged ≥22.22.1 vs 20.20.2)                            |
| 2026-08-11 12:51Z | full audit matrix     | lint/build/test/coverage/prettier/security | lint 0/0 · build PASS (27ms) · JS 1087 pass/4 skip · pytest 13/13 (post-install) · cov 95.19/92.91/97.14 |
| 2026-08-11 12:52Z | security scan         | `check-workflow-security.js`               | 12 violations — 2 CRITICAL + 10 HIGH (33rd)                                                              |
| 2026-08-11 12:52Z | freshness + CI probes | `check-freshness.js` / `gh run list`       | STALE 22d (F018) · orchestrator 5/5 failure (F063) · SITE_URL placeholder (R-132-2)                      |
| 2026-08-11 12:53Z | F002 probe            | `gh issue create`                          | GraphQL createIssue denied — blocked (120th)                                                             |
| 2026-08-11 12:54Z | records written       | docs/issues/2026-08-11/ (18/19/20)         | delta audit + delta issue records + decision                                                             |
