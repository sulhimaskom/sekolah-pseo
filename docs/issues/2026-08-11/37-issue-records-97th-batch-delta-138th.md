# Issue Records — 97th Batch (Delta, 138th verification, 2026-08-11)

**Evaluation Date**: 2026-08-11
**Evaluator**: Sisyphus (ULW Loop, autonomous run)
**Basis**: 137th audit matrix on `5bcf3e9` (see `36-audit-report-2026-08-11-138th.md`).
**Convention**: GitHub issue creation is denied for this integration token (F002 —
`GraphQL: Resource not accessible by integration (createIssue)`, 126th consecutive
denial). Per the 135-run docs-only convention, findings are recorded in this ledger.

## Stability confirmation (no action — held findings)

All labeled findings from the 96th-batch records remain HELD with byte-identical
...
| F037 | security/P0       | HELD   | duplicate `GEMINI_API_KEY` API_KEY refs — CRITICAL (39th)                  |
...
| F002 | chore/P1          | HELD   | `gh issue create` denied (126th)                                           |
...
| F004 | security/P2       | HELD   | `secrets.*` refs across workflow envs (59 refs / 10 unique names)          |
...
### R-138-1: Build timing variance within budget (chore, P3)

- **Finding**: full build 29ms / 68.97 pages/sec this run vs 32ms / 62.5 pages/sec at
  the 137th — run-to-run noise on a 2-school sample; budgets still PASS.
- **Files affected**: none (observation only).
- **Labels**: `chore`, `P3`.

### R-138-2: SITE_URL placeholder warning re-observed (chore, P2)
...
- **Evidence**: `node scripts/check-freshness.js` → "SITE_URL is set to default
  placeholder \"https://example.com\"" (re-observed 10th run). Config reference:
...
### R-138-3: orchestrator failure sample consistent (ci, P1)

- **Finding**: last 6 scheduled `oc - orchestrator` runs (2026-08-06 → 2026-08-11) all
  `failure` — consistent with the 137th's 6/6 sample. Root cause unchanged: checkout
...
### R-138-4: F068 pytest re-observed, resolved in-loop (test, P2)
...
### R-138-5: workflow-security violations re-sampled in detail (security, P0)

- **Finding**: `node scripts/check-workflow-security.js` exits 1 with **12 violations**
  — 2 CRITICAL + 10 HIGH (**39th consecutive observation** of F037/F038):
