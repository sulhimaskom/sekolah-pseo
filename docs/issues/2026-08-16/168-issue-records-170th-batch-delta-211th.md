# Issue Records — 170th Batch (Delta, 211th verification, 2026-08-16)

**Evaluation Date**: 2026-08-16
**Evaluator**: Sisyphus (ULW Loop, autonomous run)
**Basis**: 210th audit matrix on `c655d24`, re-executed fresh this run (see
`167-audit-report-2026-08-16-211th.md`).
**Convention**: GitHub issue creation is denied for this integration token (F002 —
`GraphQL: Resource not accessible by integration (createIssue)`, **199th
consecutive denial**, freshly probed this run). Per the 207-run docs-only
convention, findings are recorded in this ledger. Each entry carries the
mandated category + priority labels (contract §4).

## This run: 0 open PRs / 0 open issues → Phase 1 audit only — composite **70.4 (±0.0, 15th flat)**, no new findings, no source delta; F037 112th obs, F038 42d, F002 199th, F005 99/59th, F018 27d held, F008 RESOLVED maintained (41L + 11 modules), F029 NOT re-observed, 1121 JS + 27 py green, coverage 95.57/93.07, pull CI window 13/1/1 with 47th+48th+49th+50th+51st+52nd+53rd+54th+55th+56th+57th+58th windows SUCCESS ×12 back-to-back (59th win in-prog, 46th-window infra failure re-confirmed), pytest 13/13 + run_tests 27/27

## Open findings (held)

| ID   | Category    | Priority | Title                                                                                                                                                                                           | State            |
| ---- | ----------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------- |
| F037 | security    | P0       | Workflow security: 12 violations (2 CRITICAL DUPLICATE_API_KEY + 10 HIGH)                                                                                                                       | HELD (112th obs) |
| F038 | ci          | P1       | Orchestrator workflow fails 10/10 (checkout exit 128, 42 days)                                                                                                                                  | HELD (42 days)   |
| F002 | chore       | P1       | GitHub issue creation denied for integration token (199th denial)                                                                                                                               | HELD             |
| F044 | security    | P2       | Over-scoped secrets: 10 unique names / 59 refs across workflows + template                                                                                                                      | HELD             |
| F005 | chore       | P2       | 99 docs/issues ledger files fail `prettier --check` (59th flat, held)                                                                                                                           | HELD/FLAT        |
...
| F063 | ci          | P1       | pull CI hourly — 13/1/1; 46th-window failure infra-class (`Endpoint is unavailable`, re-verified via run logs); 47th+48th+49th+50th+51st+52nd+53rd+54th+55th+56th+57th+58th windows SUCCESS ×12 | WATCH            |
...
`d77121e..c655d24` is docs-only, so no new source surface exists): duplication
...
in a clean, buildable, green state (see `167-audit-report-2026-08-16-211th.md`).
