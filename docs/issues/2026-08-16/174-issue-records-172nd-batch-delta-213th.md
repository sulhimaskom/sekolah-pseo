# Issue Records — 172nd Batch (Delta, 213th verification, 2026-08-16)

**Evaluation Date**: 2026-08-16
**Evaluator**: Sisyphus (ULW Loop, autonomous run)
**Basis**: 212th audit matrix on `c0fbcd6`, re-executed fresh this run (see
`173-audit-report-2026-08-16-213th.md`).
**Convention**: GitHub issue creation is denied for this integration token (F002 —
`GraphQL: Resource not accessible by integration (createIssue)`, **201st
consecutive denial**, freshly probed this run). Per the 209-run docs-only
convention, findings are recorded in this ledger. Each entry carries the
mandated category + priority labels (contract §4).

## This run: 0 open PRs / 0 open issues → Phase 1 audit only — composite **70.4 (±0.0, 17th flat)**, no new findings, no source delta; F037 114th obs, F038 43d, F002 201st, F005 99/61st, F018 27d held, F008 RESOLVED maintained (41L + 11 modules), F029 NOT re-observed, 1121 JS + 27 py green, coverage 95.57/93.07, pull CI window **14/0/1 zero-failure maintained** with 48th+49th+50th+51st+52nd+53rd+54th+55th+56th+57th+58th+59th+60th+61st windows SUCCESS ×14 back-to-back (46th+47th windows rolled off; 62nd win in-prog), F063 IMPROVING, pytest 13/13 + run_tests 27/27

## Open findings (held)

| ID   | Category    | Priority | Title                                                                                                                                                                                                                                                                   | State            |
| ---- | ----------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------- |
| F037 | security    | P0       | Workflow security: 12 violations (2 CRITICAL DUPLICATE_API_KEY + 10 HIGH)                                                                                                                                                                                               | HELD (114th obs) |
| F038 | ci          | P1       | Orchestrator workflow fails 10/10 (checkout exit 128, 43 days)                                                                                                                                                                                                          | HELD (43 days)   |
| F002 | chore       | P1       | GitHub issue creation denied for integration token (201st denial)                                                                                                                                                                                                       | HELD             |
| F044 | security    | P2       | Over-scoped secrets: 10 unique names / 59 refs across workflows + template                                                                                                                                                                                              | HELD             |
| F005 | chore       | P2       | 99 docs/issues ledger files fail `prettier --check` (61st flat, held)                                                                                                                                                                                                   | HELD/FLAT        |
...
| F063 | ci          | P1       | pull CI hourly — **14/0/1, zero-failure window maintained** (46th-window 20:14Z infra failure + 47th-window rolled off); 48th+49th+50th+51st+52nd+53rd+54th+55th+56th+57th+58th+59th+60th+61st windows SUCCESS ×14 back-to-back; F038 remains the structural CI failure | IMPROVING        |
...
`a16a0ed..c0fbcd6` is docs-only, so no new source surface exists): duplication
...
in a clean, buildable, green state (see `173-audit-report-2026-08-16-213th.md`).
