# Issue Records — 175th Batch (Delta, 216th verification, 2026-08-16)

**Evaluation Date**: 2026-08-16
**Evaluator**: Sisyphus (ULW Loop, autonomous run)
**Basis**: 215th audit matrix on `a1adb1a`, re-executed fresh this run (see
`182-audit-report-2026-08-16-216th.md`).
**Convention**: GitHub issue creation is denied for this integration token (F002 —
`GraphQL: Resource not accessible by integration (createIssue)`, **204th
consecutive denial**, freshly probed this run). Per the 212-run docs-only
convention, findings are recorded in this ledger. Each entry carries the
mandated category + priority labels (contract §4).

## This run: 0 open PRs / 0 open issues → Phase 1 audit only — composite **70.4 (±0.0, 20th flat)**, no new findings, no source delta; F037 117th obs, F038 46d, F002 204th, F005 99/64th, F018 27d held, F008 RESOLVED maintained (41L + 11 modules), F029 NOT re-observed, 1121 JS + 27 py green, coverage 95.57/93.07, pull CI window **14/0/1 zero-failure maintained** with 50th+51st+52nd+53rd+54th+55th+56th+57th+58th+59th+60th+61st+62nd+63rd+64th+65th windows SUCCESS ×16 back-to-back (66th win in-prog), F063 IMPROVING, pytest 13/13 + run_tests 27/27

## Open findings (held)

| ID   | Category    | Priority | Title                                                                                                                                                                                                                                     | State             |
| ---- | ----------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------- |
| F037 | security    | P0       | Workflow security: 12 violations (2 CRITICAL DUPLICATE_API_KEY + 10 HIGH)                                                                                                                                                                 | HELD (117th obs)  |
| F038 | ci          | P1       | Orchestrator workflow fails 10/10 (checkout exit 128, 46 days)                                                                                                                                                                            | HELD (46 days)    |
| F002 | chore       | P1       | GitHub issue creation denied for integration token (204th denial)                                                                                                                                                                         | HELD              |
| F044 | security    | P2       | Over-scoped secrets: 10 unique names / 59 refs across workflows + template                                                                                                                                                                | HELD              |
| F005 | chore       | P2       | 99 docs/issues ledger files fail `prettier --check` (64th flat, held)                                                                                                                                                                     | HELD/FLAT         |
...
| F063 | ci          | P1       | pull CI hourly — **14/0/1, zero-failure window maintained**; 50th+51st+52nd+53rd+54th+55th+56th+57th+58th+59th+60th+61st+62nd+63rd+64th+65th windows (04:28Z → 17:13Z) SUCCESS ×16 back-to-back (66th = this run in-prog); F038 remains the structural CI failure | IMPROVING         |
...
No new findings this run — every candidate surfaced by the two parallel deep
scans was re-matched to existing held/resolved ledger entries this window (git
history is docs-only through `a1adb1a`, so no new source surface exists):
duplication clusters → F019/F048; workflow security & unused composite action →
F037/F044/F007; missing automated CI gates → F066/F068; pre-commit filter
mismatch → F005/F064 cluster; devcontainer/requirements gaps → F044/F064. No
two open entries describe the same defect; all retained entries carry exactly
one category + one priority label.
...
in a clean, buildable, green state (see `182-audit-report-2026-08-16-216th.md`).