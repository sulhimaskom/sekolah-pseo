...
| Coverage                 | ✅ PASS | Statements 92.22%, Branches 90.89%, Functions 94.55%, Lines 92.16% |
...
- A. Code Quality: +0.30 (better test coverage data + regression fix + security hardening)
- B. System Quality: +0.55 (reduced CI secret exposure + workflow permissions fixed)
- C. Experience Quality: 0.00 (unchanged)
- D. Delivery & Evolution Readiness: +0.75 (CI/CD health improved, permissions fixed)
- **Overall Delta: +0.40** (improved via security hardening and test regression fix)

---

## Applied Fixes

During this audit run, two issues were fixed:

### Fix 1: `getUniqueDirectories` regression in PageBuilder.js

The refactoring in commit `d6ec7db` made `getUniqueDirectories` delegate to `getSchoolRelativePath()`, which validates ALL school fields including `npsn` and `nama`. But `getUniqueDirectories` only needs directory-level fields (`provinsi`, `kab_kota`, `kecamatan`). Fixed by wrapping the call in try-catch — invalid schools are skipped during directory pre-creation; their validation failure is handled downstream by `writeSchoolPagesConcurrently`.

- **Tests restored**: 902/902 pass (was 900/902)

### Fix 2: Workflow hardening

1. **Added `issues: write`** to `on-push.yml` permissions block (was missing — blocked issue creation).
