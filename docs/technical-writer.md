...
- TASK-103 lesson (2026-08-17): `docs/setup.md`'s Project Structure tree is the most drift-prone doc — it was NOT updated by TASK-094's layer separation (8 infra modules still listed under `scripts/` after moving to `src/core/`) nor by TASK-101's shared components. Unlike README/blueprint (updated in TASK-094), setup.md's tree was silently stale. When a task moves/creates files, always check: README.md tree, docs/setup.md tree, docs/blueprint.md tree, api.md Module Organization tree — four places, not three. Also: blueprint.md's Environment Variables table drifts independently of .env.example — cross-check against `grep process.env src/` (LOG_LEVEL in logger.js, ENRICHMENT_ENABLED in enrichment.js were both missing).
